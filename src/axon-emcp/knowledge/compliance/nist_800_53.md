...
summary: AXON annotations covering the NIST 800-53 Rev. 5 control catalogue — control families AC, AU, SC, SI, IR, CM, CP, IA mapped to compiler + runtime enforcement.
---
...
travels with FISMA + a FedRAMP baseline:
...
|---|---|---|
| AC-2 — account management | Lifecycle of system accounts | OIDC layer (§40.w); audit row on every account event |
...
| AU-9 — protection of audit info | Tamper-evidence | Runtime — hash-linked + signed chain (§27.k FIPS-friendly) |
...
| Control | What it requires | AXON enforcement |
|---|---|---|
| SI-4 — system monitoring | Continuous monitoring | Runtime — §40 observability layer |
| SI-7 — software/firmware/info integrity | Detection of unauthorised changes | Runtime — hash-linked audit chain |
| SI-10 — info input validation | Validate inputs | Compile time — §40 D4 body schema validation activates on NIST-tagged endpoints with refined types |
| SI-15 — info output filtering | Validate outputs | Compile time — §40 D5 output schema validation |
...
| IR-5 — incident monitoring | The §40 observability layer surfaces incident metrics |
...
| Control | AXON enforcement |
|---|---|
| IA-2 — identification + authentication (org users) | Runtime — OIDC layer, MFA enforced per the §40.w policy |
...
axonendpoint CaseLookup {
    ...
    compliance: [NIST_800_53, FedRAMP_Moderate]
    mandate: StrictLeastPrivilege
}
```
