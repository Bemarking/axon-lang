...
| **7.a** | **Verticals — 4 new templates** (`legaltech`, `fintech`, `pharmatech`, `medic_research`) + Domain enum 8→12 + classifier curation + `parse_domain_hint` aliases. Drift gate caught the `severity` reserved-keyword collision in type-body fields. Coverage: 12/33 templates. | ✅ |
| **7.b** | Agent patterns — 8 new (chat_research, chat_tools, chat_skills, whatsapp, voice, dev, sales_consultive, sales_widget) | next |
| **7.c** | Application patterns — 13 new (workflow, BI, corporate, self_learning, document, ticket, content_mod, knowledge, compliance, recruitment, education, financial_advisor, data_pipeline) | |
