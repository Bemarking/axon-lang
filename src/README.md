...
| **6.b** | **Tier 1 — 10 primitives documented** (`context`, `intent`, `memory`, `agent`, `probe`, `validate`, `refine`, `weave`, `type`, `run`) + 12 canonical `.axon` examples drift-gated through the live `axon-frontend` pipeline. Coverage now **17/47 Documented (36%)**. The drift gate caught 3 first-draft schema bugs before merge (`memory.store:` is the lifecycle catalog, not store-kind). | ✅ |
| **6.c** | Tier 2 — 13 primitives (`resource`, `fabric`, `manifest`, `observe`, `reconcile`, `lease`, `ensemble`, `session`, `axonstore`, `dataspace`, `corpus`, `pix`, `taint`) | next |
