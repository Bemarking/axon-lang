...
| 25.c OTS native transformers → C (μ-law + resample) | ⏳ pending | ~700 | C + Rust shim | Port de `axon-rs/src/ots/native/{mulaw,resample}.rs` a `axon-csys/c-src/audio/{mulaw,resample}.c`; SIMD intrinsics donde available (SSE2/AVX2/NEON); ~30 tests |
| 25.d Buffer pool slab allocator → C | ⏳ pending | ~1200 | C + Rust shim | Port del slab allocator de `axon-rs/src/buffer/pool.rs` a C23 con `alignas(64)` cache-line + huge-pages opt-in (`MAP_HUGETLB` Linux / `MEM_LARGE_PAGES` Windows) + bitmap free-list con `__builtin_ctzll`; per-tenant byte budget enforcement; ~40 tests |
| 25.e Effects FSM dispatch → C (computed gotos) | ⏳ pending | ~1500 | C + Rust shim | Port del dispatcher de `axon-rs/src/effects/runtime.rs` a C23 con computed gotos (`goto *labels[op]`); paper §5 "operaciones atómicas de salto" delivered; benchmark target ≥10× speedup vs tree-walker; ~50 tests |
| 25.f C transpiler for compute blocks (adopter-facing) | ⏳ pending | ~500 | Python | Nueva clase `CTranspiler` paralelo a `RustTranspiler` en `axon/runtime/c_transpiler.py`; `compute Foo { backend: c, logic: ... }` adopters opt-in; cc-shell or `tcc` for hot-reload; ~25 tests |
