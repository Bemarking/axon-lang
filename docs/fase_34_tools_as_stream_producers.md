...
status: ⏳ DRAFT — pending founder bloque ratification on D1–D13. Depends strictly on Fase 33.z SHIPPED v1.27.0 (production dispatcher wiring; the dispatcher arm graduation in 34.e requires the dispatcher to be ON in production first).
owner: AXON Runtime + Backends Team
created: 2026-05-13
target: axon-lang v1.28.0 (minor — the `Tool` trait gains a `stream(args) → Stream<ToolChunk>` surface; the tool registry exposes a streaming dispatch; the 4 disjunctions of `produces_stream(F)` — `output: Stream<T>` / `apply: <stream-tool>` / `use_tool: <stream-tool>` / `perform Stream.Yield` — converge on a SINGLE chunk-level wire behavior; existing tools default to single-chunk streaming for D9 backwards-compat)
...
|---|---|---|---|---|
| **34.a** | spec + diagnostic anchor | ~700 | ⏳ pending bloque | This plan vivo + new `axon-rs/tests/fase34_a_unified_stream_handler_diagnostic.rs` (~500 LOC). 4 representative flow shapes (one per disjunction (a)/(b)/(c)/(d)) + assert v1.27.0 baseline: disjunctions (a) and (d) emit per-chunk wire; disjunctions (b) and (c) emit `axon.tool_call` once + then a single materialized chunk (the post-33.z partial state). Each subsequent sub-fase INVERTS the relevant anchor assertion. Plus 1 catalog totality pin asserting the 4 disjunctions are the closed set (no 5th `produces_stream` rule sneaks in). |
...
| **34.l** | Adopter docs | ~800 | ⏳ pending bloque | New `docs/MIGRATION_v1.28.md` (~500 LOC) with 5 scenario recipes: (A) server-only upgrade — stream-tools auto-stream once you declare `<stream:<policy>>`; (B) implementing a custom streaming tool — adopter writes a `Tool::stream()` impl; (C) per-chunk audit for regulated-vertical replay; (D) cross-stack — Python Tool::stream parity; (E) backwards-compat — existing tools keep working byte-equal. `docs/ADOPTER_STREAMING.md` extension (~300 LOC) with new §"Tools as stream-producers (Fase 34, v1.28.0+)" mapping D1–D13 to adopter-observable behavior. The §"What the paper means by stream-effect tools" canonical section finally lands. |
| **34.m** | release v1.28.0 cross-stack + axon-enterprise v1.19.0 catch-up | release | ⏳ pending bloque | axon-lang v1.27.0 → v1.28.0 cross-stack: bump-my-version minor across 6 files. axon-frontend bumps from 0.11.1 → 0.12.0 if Tool trait surface requires AST changes (TBD; may stay 0.11.1 if the trait is runtime-only). axon-enterprise v1.19.0 substantive release: vertical-grade `Tool::stream()` implementations land in `axon_enterprise.shield` (HIPAA PHI scrubber streamer + legal privilege scanner streamer + fintech AML pipeline streamer + government audit trail streamer) — NOT identity placeholders, real per-chunk processing. PyPI + crates.io + GitHub Release with content-first notes. |
...
**What v1.28.0 (Fase 34) ships**: tools become first-class stream-producers. The `Tool::stream()` trait method exists; the registry derives `is_streaming` from declarations; the dispatcher invokes `tool.stream()` for stream-tools; the 4 disjunctions of `produces_stream(F)` converge on a single `unified_stream_handler`; cancel propagates into tool bodies at p95 ≤100ms; per-chunk audit captures the actual tool execution trail.

**What v1.28.0 does NOT ship**:
...
2. **v1.28.0 (34)**: tools themselves become stream producers. Adopters who declare `<stream:<policy>>` on a tool see per-tool-chunk wire emission.

Adopters upgrading v1.26.0 → v1.27.0 → v1.28.0 in sequence see:
...
- v1.28.0 hop: their stream-tool declarations finally activate at runtime — per-tool-internal-chunk wire emission, cancel into tool body, full audit.

Adopters who jump v1.26.0 → v1.28.0 get both transitions at once. Both hops are strictly additive (D4 byte-compat on canonical shapes; D9 backwards-compat on existing tools).
