...
status: ✅ CLOSED 2026-05-20 — axon-lang v1.38.5 + axon-enterprise v1.29.4 LIVE cross-stack.
...
| **37.y.8** | Coordinated patch release axon-lang **v1.38.5** + axon-frontend **0.19.3** (additive AST + IR fields + new compile error). axon-enterprise **v1.29.4** catch-up per the standing rule. | — | ✅ SHIPPED 2026-05-20 — cross-stack release LIVE end-to-end. **axon-lang v1.38.5**: release commit `921bd60` + 3 tags pushed (`v1.38.5`, `rust-v1.38.5`, `axon-frontend-v0.19.3`); crates.io published (axon-frontend 0.19.3 first → axon-lang 1.38.5 second; ordering preserves the build-time dep pin `axon-frontend = "=0.19.3"`); GitHub Release published with content-first notes (5 D-letters + market-vs-axon table + vertical-relevance + honest scope); PyPI publish.yml fired cleanly on `release: published` event (no draft-toggle recovery needed — first since v1.38.4); PyPI v1.38.5 live + visible. **axon-enterprise v1.29.4**: PR #42 merged commit `ed10a3d` (2-file diff: `pyproject.toml` 1.29.3→1.29.4 + dep pin `axon-lang>=1.38.4`→`>=1.38.5` + `axon_enterprise/__init__.py` `__version__`); tag `v1.29.4` pushed via refspec mapping `enterprise/v1.29.4:refs/tags/v1.29.4`; GitHub Release v1.29.4 published with vertical-inheritance notes (HIPAA + FRE 502 + BSA-OFAC + FedRAMP each get the natural REST surface); Enterprise Release Docker build + ECR Private image pushed clean in 8m13s with two-phase CDN-consistency wait (per PR #14 commit `9d7fe12` hardening); axon-csys-enterprise workflow green; Fase 29 Vertical Diagnostic Enhancements workflow green on both master + tag. **PyPI CDN propagation race** caught + recovered: PR #42 CI first failed on 6 lanes (Telemetry sink + Vertical policy + Vertical suggest dictionaries + Full diagnostics — all consuming the new `axon-lang>=1.38.5` dep); `gh run rerun --failed` fired twice (first burned cache while PyPI CDN still propagating, second hit the warm CDN edge); 24/24 lanes green on second rerun. **Cumulative regression**: 442/442 axon-frontend lib + 2104/2104 axon-rs lib + 8/8 Fase 37.y anchor green; zero regressions cross-stack. Founder standing rule honored end-to-end: every axon-lang release ships an axon-enterprise catch-up in lockstep. |
