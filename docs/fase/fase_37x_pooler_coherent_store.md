...
| **D9** | **Self-healing, bounded schema cache.** The `(dsn, table) → (schema, column_types)` cache is capacity-bounded (LRU — the 10k bound the idempotency/replay caches already use) so a many-table / many-DSN / multi-tenant adopter cannot grow it unbounded. It SELF-HEALS against live schema drift: an operation that fails with a schema-drift SQLSTATE — `42P01 undefined_table`, `42703 undefined_column`, `42804 datatype_mismatch` — evicts the stale `(dsn, table)` entry and retries the operation ONCE against fresh introspection. The retry is provably safe: those three SQLSTATEs are parse/plan-time rejections — the statement had ZERO side effects — so a retried `persist`/`mutate` cannot double-write. A live `ALTER TABLE` during server uptime can no longer poison the cache until a process restart. |
...
| **37.x.b** | **Search-path-independent resolution (D1)** — replace `column_types(table) -> Map` with `resolve_table(table) -> Result<(schema, Map)>`: `to_regclass($1)` primary; a `pg_class`⋈`pg_namespace` `relname` scan as the fallback; exactly-one-match resolves, zero-match and unbreakable-ambiguity are typed errors (feeds D6). The `pg_attribute`⋈`pg_type` introspection keys on the resolved relation OID. New `StoreError` variants for the unresolved/ambiguous cases. | OSS | D1 | ⏳ pending |
| **37.x.c** | **Schema-anchored operation SQL (D2)** — `build_select_sql` / `build_insert_sql` / `build_update_sql` / `build_delete_sql` emit `"schema"."table"`; the schema identifier is `is_safe_identifier`-validated and double-quoted. The `(dsn, table)` cache value becomes `(schema, column_types)`; an unresolved table is never cached. Pure-builder unit tests updated for the qualified form. | OSS | D2, D5 | ⏳ pending |
| **37.x.d** | **One coherent session (D3)** — `query` / `insert` / `mutate` / `purge`: on a cache miss, one `pool.begin()` transaction wraps `resolve_table` + the operation. `stream_retrieve` holds the transaction for the cursor drain's lifetime — bounded by the `PauseUpstream` row cap, so a held pooler backend is time-bounded (no pool starvation). Cache hit ⇒ no transaction. Gap-3 `statement_cache_capacity(0)` + `application_name` preserved. The four public async signatures unchanged (D5). | OSS | D3, D5 | ⏳ pending |
| **37.x.e** | **Equality type-agnostic fallback (D4)** — `build_pg_where`: an unknown-type column under `==`/`!=` renders `"col"::text = $N`; ordering + `LIKE` keep the bare `$N`; the `NULL`-fold path is unaffected. Shipped + documented as an explicitly DEGRADED best-effort backstop (exact only for canonical-form inputs) — the load-bearing path is D1+D8 introspection. Exhaustive unit tests over {known, unknown} × {equality, ordering, LIKE, NULL}. | OSS | D4 | ⏳ pending |
| **37.x.f** | **Self-healing, bounded schema cache (D9)** — `SCHEMA_CACHE` becomes a capacity-bounded LRU (10k); an operation failing with `42P01`/`42703`/`42804` evicts the `(dsn, table)` entry and retries once against fresh introspection. The retry-safety proof (parse/plan-time SQLSTATEs ⇒ zero side effects ⇒ no double-write) is pinned in tests. | OSS | D9 | ⏳ pending |
