...
status: ✅ CLOSED 2026-05-22 — v2.0.0/v2.0.1 live cross-stack (crates.io + PyPI + GitHub Releases); enterprise catch-up + adopter migration handed to Fase 40. Charter ratified by founder 2026-05-21.
...
| **39.i** ✅ SHIPPED 2026-05-22 | Atomic deploy v2.0.0 + axon-enterprise pin-cap + Kivi handoff | coordinated `git push origin master + tag v2.0.0`, `cargo publish axon-frontend v1.0.0` + `cargo publish axon-lang v2.0.0`, PyPI binary upload, GitHub Release with the migration guide, axon-enterprise PR + tag, Kivi adopter `.axon` source updated (`output: T` → `output: FlowEnvelope<T>` for affected endpoints) | ✅ **THE ATOMIC DEPLOY — v2.0.0 + v2.0.1 LIVE CROSS-STACK**. Executed step-by-step per founder directive ("ejecutando todo el proceso pero paso a paso y deteniendo en el paso 3 para verificar que los binarios estén presentes"). **crates.io**: `axon-frontend v1.0.0` (MAJOR — `Cardinality::Wrapped` enum) + `axon-csys v0.2.0` (new envelope Theorem-5.1 kernel) + `axon-lang v2.0.0` then `v2.0.1` all published. **GitHub Releases**: `v2.0.0` + `v2.0.1` with all 6 platform binary archives (linux x86_64/aarch64, macos x86_64/aarch64, windows x86_64) + `.sha256`. **PyPI**: `axon-lang 2.0.1` with the FIXED binary-wrapper (`_bootstrap.py` downloads+extracts the release archive — D13 distribution shim); verified end-to-end via clean `pip install axon-lang==2.0.1` → `axon version` → "axon-lang 2.0.1". **The step-3 verification checkpoint was vindicated** — it caught BUG #1 (bootstrap downloaded raw binary `axon-linux-x86_64` 404 instead of the `.tar.gz` archive; `platform.system()` returns "Darwin" not "macos") BEFORE any user was affected. 3 further bugs caught by per-step verification: BUG #2 (cargo publish axon-lang 2.0.0 verify-fail — axon-csys envelope module added in 39.c but never re-published → bumped 0.1.2→0.2.0), BUG #3 (PyPI 2.0.0 shipped the pre-fix buggy wrapper from the tag commit → coordinated 2.0.1 patch since PyPI forbids version reuse; founder lockstep discipline preferred over PyPI-only desync), BUG #4 (binary printed stale "2.0.0" after bump — `cargo clean -p axon-lang` resolved CARGO_PKG_VERSION incremental staleness). **axon-enterprise**: PR #48 caps `axon-lang>=1.40.3,<2.0.0` (defensive — the Python `axon_enterprise` package imports `from axon.*` purged modules and CANNOT consume the v2.0.0 binary launcher; the real enterprise Rust migration is **Fase 40**). **Handoffs**: (1) Fase 40 = axon-enterprise Python eradication / real v2.0.0+ catch-up; (2) PyPI 2.0.0 yank = founder manual via PyPI web UI (broken wrapper superseded by 2.0.1 as latest, so `pip install axon-lang` already resolves correctly). |
...
- ✅ axon-lang **v2.0.0** (+ **v2.0.1** patch) tag pushed + crates.io
  published + GitHub Release with the FlowEnvelope migration guide
- ✅ axon-frontend **v1.0.0** tag pushed + crates.io published
  (+ axon-csys **v0.2.0** envelope kernel published)
- ✅ PyPI `axon-lang 2.0.1` binary-wrapper package published; `pip
  install axon-lang` produces a working native binary on Linux /
  macOS / Windows (verified end-to-end clean install → `axon version`)
- 🔁 **→ Fase 40 handoff**: axon-enterprise catch-up. PR #48 caps the pin
  `<2.0.0` (defensive) because the Python `axon_enterprise` package imports
  `from axon.*` purged modules and CANNOT consume the v2.0.0 binary launcher.
  A real v2.0.0+ catch-up requires the enterprise Rust migration = **Fase 40**.
- ✅ `find . -name "*.py" -not -path "./tests/*" -not -name
  "_bootstrap.py"` returns empty (the audit: zero language Python)
- 🔁 **→ Fase 40 handoff**: the single adopter's coordinated production
  migration (`output: T` → `output: FlowEnvelope<T>`) is sequenced with the
  enterprise catch-up; v2.0.1 is live + verified, adopter cutover follows.
- ✅ Memory entry for Fase 39 plan SHIPPED status updated

> **PyPI 2.0.0 yank** (founder manual via PyPI web UI): the broken 2.0.0
> wrapper is superseded by 2.0.1 as `latest`, so `pip install axon-lang`
> already resolves to the working build; yanking 2.0.0 is cosmetic hygiene.
