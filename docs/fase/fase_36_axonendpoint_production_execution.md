...
status: ✅ CLOSED 2026-05-17 — ALL 13 sub-fases 36.a–36.m SHIPPED. axon-lang v1.34.0 live cross-stack (crates.io axon-lang 1.34.0 + axon-frontend 0.16.0 + PyPI axon-lang 1.34.0 + GitHub Release v1.34.0 with 5 binary tarballs; commit `48acef1`, tags `v1.34.0`/`rust-v1.34.0`/`axon-frontend-v0.16.0`). axon-enterprise v1.25.0 catch-up live (PR #30 merged commit `d6c6bb8` + tag `v1.25.0` + Enterprise Release Docker→ECR success + GitHub Release published). D1–D12 ratificadas bloque 2026-05-17 (founder "Ratifico todos D-Letters"). Triggered by the kivi-enterprise gap report 2026-05-17 (axonendpoint SSE executes against the no-op `stub` backend; no way to wire a real LLM), verified true and deeper than reported. Rust-canonical. 93 Fase 36-specific tests + a dedicated 4-lane CI workflow (`fase_36_backend_resolution.yml`) with a 20 000-iteration resolver fuzz; zero regressions.
...
| **36.m** | Coordinated release axon-lang v1.34.0 cross-stack (crates.io + PyPI + GitHub Release + binaries) + axon-enterprise catch-up. | SPLIT | — | ✅ SHIPPED — `bump-my-version` minor 1.33.0 → 1.34.0 (6 files) + axon-frontend 0.15.0 → 0.16.0 + dep pin `=0.16.0`; release commit `48acef1`; 3 tags pushed (`v1.34.0`, `rust-v1.34.0`, `axon-frontend-v0.16.0`). `cargo publish` axon-frontend 0.16.0 + axon-lang 1.34.0 → crates.io. GitHub Release v1.34.0 with content-first notes + 5 binary tarballs (`rust_release.yml` green, 21m). PyPI axon-lang 1.34.0 published (`publish.yml` fired via draft-toggle recovery — the `release:published` event dropped on first auto-publish). Fase 36 CI workflow green on the release commit. **axon-enterprise v1.25.0 catch-up**: branch `feature/36m-axon-lang-1.34.0-catchup`, bump 2 files + dep pin `axon-lang>=1.34.0`, PR #30, all checks green (Python Fase 29 + C-systems sanitizers/valgrind), merged commit `d6c6bb8`, tag `v1.25.0`, Enterprise Release Docker→ECR success, GitHub Release v1.25.0 published. |
