...
| Sub-fase | Scope |
|---|---|
| **41.a** | **Session-type algebra + duality + projection** in `axon-frontend` (pure): the type grammar (§3.1), the duality involution $(\cdot)^{\perp}$ + the regular-coinductive equality for $\mu$-types, the binary connection-law check $\bar S \stackrel{?}{=} S^{\perp}$. Property-tested (involutivity, duality round-trip). |
| **41.b** | **The `socket` primitive surface**: parser + AST + IR for the `socket { protocol, backpressure, duality, reconnect, legal_basis }` block; the session-type concrete syntax (`send`/`recv`/`select`/`branch`/`rec`/`end`). Cross-stack drift gate (Rust frontend ⇄ Python wrapper). |
