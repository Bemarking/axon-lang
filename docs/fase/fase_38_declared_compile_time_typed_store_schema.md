...
status: ✅ CLOSED 2026-05-20 — axon-lang v1.38.0 live cross-stack (PyPI + crates.io axon-lang 1.38.0 + crates.io axon-frontend 0.19.0 + GitHub Release v1.38.0) + axon-enterprise v1.29.0 catch-up live (dep pin advance only, no enterprise code change). D1–D10 RATIFIED (founder bloque, 2026-05-19). **Sub-fases 38.a–38.k ALL SHIPPED** (38.b-g 2026-05-19; 38.h + 38.i + 38.j 2026-05-20). D2 compile-time proof COMPLETE for INLINE schemas (T801–T805) + D3 per-tenant env-var (T806) + D8 strengthening (T807) + D10 manifest-export CLI. All 5 pre-38 anchors §1–§5 INVERTED in place as regression guards. 38.i ships end-to-end integration (5-tenant curated manifest fixture + every D2 error code + composite suggestion + multi-store + 38.h public-API smoke) and 3-surface property/fuzz (~6 200 deterministic LCG iters covering Manifest::parse_json totality, check_filter totality, canonical_serialize fixed point). 38.j ships the load-bearing wire that activates T807 in production: `axon serve --schemas-dir <path>` flag + `AXON_SCHEMAS_DIR` env var, `ServerConfig.schemas_dir` field, `deploy_handler` manifest loading via `load_and_merge_manifests` + `verify_postgres_schemas_with_manifest(Some(&m))`, structured 200-OK error shape (`phase: store_schema_manifest_load`, `d_letter: D3+D8`, schemas_dir echoed). Plus the 5-lane Fase 38 CI workflow (pure-column-proof, manifest-roundtrip, per-tenant-resolution, declared-vs-live-drift, d5-zero-regression), `docs/MIGRATION_v1.38.md` (6 scenarios A-F), `docs/ADOPTER_AXONSTORE.md` §17 (5 recipes), new `docs/ADOPTER_TYPED_STORE.md` deep-dive (5-pillar story), and the AXON_GAP doc's "compile-time half closed" closing. **38.k ✅ SHIPPED 2026-05-20** — coordinated cross-stack release axon-lang v1.38.0 (PyPI + crates.io) + axon-frontend 0.19.0 (crates.io) + axon-enterprise v1.29.0 catch-up. Triggered by the §7 commitment Fase 37.x landed on 2026-05-19: *"the genuinely-superior axon end-state — the type-checker proves every `where:` column and every `persist`/`mutate` field against a declared/verified column schema, exactly as Fase 35 Pillar IV proves the store CAPABILITY and Fase 37 proves the request BINDING — is Fase 38: The Declared & Compile-Time-Typed Store Schema, named and committed here as the immediate next cycle."* Target axon-lang **v1.38.0** (minor — opens a new `schema:` field on `axonstore` + adds a new `axon check`-time column-type proof gate; SemVer-strict, a new declarable surface that affects compile output earns a minor). Rust-canonical (per the founder directive 2026-05-15 *"todo encaminado a ser 100% Rust + C, 0 Python"*) — the cycle lives in `axon-frontend/` (the Rust type-checker) + adds a thin runtime hook in `axon-rs/src/store/`.
...
| **38.c** | **Manifest format + resolver.** Define the canonical `.axon-schema.yml` shape (D4) — closed top-level structure, key-sorted at every level, UTF-8+LF, optional `--content-hash` header. New `axon-frontend/src/store_schema_manifest.rs` provides `parse_manifest`, `lookup(manifest, store_name) -> StoreColumnSchema`, `canonical_serialize(schema) -> String` (the byte-deterministic output `axon store introspect` produces). JSON variant supported (`.axon-schema.json`) with the same canonical discipline. Manifest discovery: `./`, `./schemas/`, and a `[axon] schema_path` in `axonproject.toml`. Content-hash verification (SHA-256 over canonical content); `axon check` flags a stale hash with `axon-T805`. 12+ unit tests covering canonical ordering, hash verification, multi-store manifests, JSON↔YAML parity. | OSS | D4, D6 | ✅ SHIPPED — new `axon-frontend/src/store_schema_manifest.rs` (~830 LOC, 100% Rust, zero new crate deps). **Manifest shape** (D4): `{"version": 1, "stores": {"<qualified.name>": {"columns": {"<col>": {"type": "<PascalCaseType>", primary_key?, auto_increment?, not_null?, unique?, default_value?}}}}, "content_hash"?: "sha256:<hex>"}`. The closed 15-type catalog from 38.b is enforced at parse time; unknown types fail with `ManifestError::UnknownColumnType` carrying a Fase 28 Levenshtein suggestion when within edit-distance 2. **Canonical serialization** is hand-rolled byte-deterministic JSON: keys sorted ASCII-ascending at every level (via `BTreeMap` storage + a custom `emit_canonical_json` traversal), no whitespace between tokens, UTF-8, no trailing newline, false-default constraints OMITTED (skip-when-default discipline matching the IR's `skip_serializing_if`). **SHA-256 hand-rolled** per FIPS 180-4 (~150 LOC, the closed `K[64]` round-constant table + the 8-word `H[0]` initial values + the 64-round compression schedule); known-vectors (`""` / `"abc"` / the 56-byte boundary case) pinned in tests so a future regression on the hash is caught. **Honest scope** documented inline: YAML support (`.axon-schema.yml`) deferred to a 38.c.2 follow-on behind an opt-in `yaml-manifest` Cargo feature pulling `serde_yaml`; the canonical-hash anchor STAYS on canonical JSON (YAML files when added parse → convert to canonical JSON → hash that, so format-mixing never breaks adopter hashes). Similarly `[axon] schema_path` in `axonproject.toml` is deferred (would require a `toml` crate dep that violates axon-frontend's zero-runtime-dep rule); the discovery layer ships with `./` + `./schemas/` lookup. **Discovery API**: `discover_manifest_files(start_dir)` walks both directories sorted-alphabetically for determinism; `load_and_merge_manifests(start_dir)` combines disjoint manifest files into one unified view + rejects cross-file duplicate store names with `ManifestError::DuplicateStore` (carries BOTH paths so the operator can fix the conflict). **Public API**: `Manifest::{new, parse_json, from_value, lookup, contains, canonical_serialize, compute_content_hash, verify_content_hash, refresh_content_hash}` + `ManifestColumn` + `ManifestStore` + `ManifestError` (6 variants — InvalidJson, InvalidStructure, UnsupportedVersion, UnknownColumnType, ContentHashMismatch (axon-T805), DuplicateStore). **Tests**: 15 module-level unit tests (parse-with-arbitrary-key-order, missing-version, unsupported-version, unknown-type with Levenshtein, unknown-type out-of-distance with empty hint, canonical-determinism pinning the exact byte form, false-constraint-omission, hash-stability across whitespace+key-order, hash-verify-passes, hash-verify-fails, hash-verify-noop-when-absent, multi-store-roundtrip, lookup-by-qualified-name, alias-normalisation-on-input, SHA-256 FIPS test vectors) + 8 integration tests in new `axon-frontend/tests/fase38_c_manifest_discovery.rs` (discovery in root + ./schemas/, sorted-determinism, empty-dir, missing-schemas-subdir, disjoint-merge, duplicate-rejection, per-file-hash-verification-before-merge, programmatic-construct-then-roundtrip mirroring 38.h's CLI shape). **Verification**: 285 axon-frontend lib (+15 manifest) + 4 fase28 + 5 fase38_a + 1+11 fase38_b + 8 fase38_c = **314 axon-frontend tests green**; axon-rs lib + 7 store integration targets untouched. Zero regressions, zero new crate deps, zero Python work (Rust-canonical from 38.b onward as committed). |
| **38.d** | **`axon check` column-type proof for `where:` (D2 first half).** New `axon-frontend/src/store_column_proof.rs` ships `StoreColumnProof::check_filter(filter_ast, declared_columns)`. Reuses the existing 35.b `parse_filter` AST (no new parser surface). Three error codes: `axon-T801` (unknown column) with Levenshtein ≤2 "Did you mean X?" suggestions (max 3); `axon-T802` (column-type mismatch) naming declared vs observed type + the parameter or literal that caused the mismatch; `axon-T805` (manifest hash mismatch — fires before the column proof, so a stale manifest gives the right error). The proof skips silently when no `schema:` is declared (D5 absolute). Every error is a Fase 28 source-context block + JSON output + LSP-compatible. 30+ unit tests covering known-good × unknown-column × type-mismatch × NULL-comparison × bound-`${param}` × literal × LIKE × `IS NULL` × all 15 catalog types. Anchor §1 + §2 INVERTED in place. | OSS | D2, D6, D5 | ✅ SHIPPED — new `axon-frontend/src/store_column_proof.rs` (~970 LOC, 100% Rust, zero new crate deps). **Architectural correction**: the runtime `parse_filter` (in `axon-rs/src/store/filter.rs`) interpolates `${param}` AT PARSE TIME against runtime bindings — losing the parameter-name info D2's type-mismatch proof needs. 38.d therefore ships a focused proof-purpose `scan_where` (state-machine tokenizer + predicate parser, ~250 LOC) that preserves `${param}` references AS `BoundParam` refs (no interpolation). The scanner is a STRICT SUBSET of the runtime grammar — recognises the same operator + literal + connector set; a `where:` syntactic miss the runtime would reject is also silently skipped here so the runtime parser owns the canonical syntactic error. **3 error codes** as planned: `axon-T801UnknownColumn` (unknown column with Fase 28 Levenshtein ≤2 "Did you mean X?" hint via `smart_suggest::suggest_for`), `axon-T802TypeMismatch` (literal-vs-column / bound-param-vs-column / LIKE-vs-non-text mismatch), `axon-T805ManifestHashMismatch` (propagated from the 38.c manifest layer). **Public API**: `ProofErrorCode` (3 variants with `.slug()` returning `axon-T801`/`axon-T802`/`axon-T805`) + `ProofError` (line+column for Fase 28 source-context anchoring) + `DeclaredColumn` + `ColumnSet::{from_inline_schema, from_inline_columns, from_manifest_store, contains, get, names}` + `FlowParamTypes::{new, insert, get}` + `scan_where` + `WhereOp` (8 variants) + `WhereValue` (BoundParam/Literal/NullKeyword) + `LiteralKind` (Text/Int/Float/Bool) + `check_filter(where_expr, columns, flow_params, where_loc) -> Vec<ProofError>` + `literal_compatible_with_column` (the closed compat matrix) + `axon_param_compatible_with_column` (the parameter-side matrix with Optional<T>/Option<T> stripping) + `load_columns_for_schema` (form-aware: inline OR manifest_ref via `Manifest::lookup` OR env_var via first-match heuristic — exact `<env_var>.<store_name>` first, then any `*.<store_name>`) + `load_manifest_from_source_dir` (best-effort via 38.c's discovery layer). **Compat matrix highlights**: String/Text param → any column (mirroring the runtime D4 fallback); UUID column accepts Text literals (canonical UUID format); LIKE requires Text/Json/Jsonb column class. **Type-checker integration**: `TypeChecker` gains `store_inline_column_sets: HashMap<String, ColumnSet>` populated during `register_declarations` for every `axonstore` carrying an inline `schema:` block + `current_flow_params: FlowParamTypes` set at `check_flow` entry; `check_flow_steps` invokes new `run_38d_where_proof` for every `Retrieve`/`Mutate`/`Purge` step; each `ProofError` converts to a `TypeError` via the existing `emit` channel. Forms (b)/(c) silently skip at the type-checker layer per honest scope — they need filesystem context (the `axon check` working dir → 38.c's `discover_manifest_files`) which lands when 38.h/38.j plumb the source-file path into the TypeChecker constructor. **Honest-scope deferral documented**: forms (b)/(c) compile-time proof requires filesystem context; the load-bearing 80% adopter use case is inline (form a) which 38.d covers fully; forms (b)/(c) are still proven at deploy via 37.x D8 (extended in 38.f). **42 unit tests** in `store_column_proof::tests` covering: scanner happy paths (empty, eq-literal, quoted-string, bound-param braced/bare, bool, float, all 6 ops, LIKE case-insensitive, IS NULL, IS NOT NULL, AND, OR), scanner malformed paths (unterminated string, trailing connector, reserved-word in column position), T801 unknown column (with + without Levenshtein hint), known-column-passes-silently, T802 literal × type matrix (Int→Text rejects, Bool→Uuid rejects, Text→Uuid passes canonical-form, Int→Int passes, Float→Numeric passes, Bool→Bool passes), T802 LIKE × non-text rejects, LIKE × Text passes, T802 bound-param matrix (String→Uuid passes via D4-mirror, Int→Uuid rejects with T802, Int→Int passes, undeclared-param silent, Optional<Int> unwraps), NULL handling (IS NULL passes any column, IS NULL against unknown column still flags T801), multi-predicate independent proof, multiple-errors-in-one-expression all reported, every-catalog-type-accepts-string-literal-or-int-or-bool (15-type smoke), every-catalog-type-accepts-String-param (D4 mirror smoke), form (b) manifest_ref resolves against provided manifest, form (b) returns None when manifest absent, form (c) env_var first-match heuristic, form (c) exact-key wins over fallback, error-code-slugs-match-namespace. **Anchor §1 + §2 INVERTED in place** in `fase38_a_typed_store_diagnostic.rs`: §1 (`tenantid` typo) now asserts `axon-T801` + the `tenant_id` suggestion is surfaced; §2 (`Int` param → `Uuid` column) now asserts `axon-T802` + names the parameter + the type mismatch. Both are now regression guards for the live proof. §3 (persist field typo — 38.e's domain) remains pre-inversion. **Verification — zero regressions**: 327 axon-frontend lib (+42 from `store_column_proof::tests`) + 4 fase28 + 5 fase38_a (with §1+§2 inverted) + 11 fase38_b_schema_grammar + 1 fase38_b_schema_drift_gate + 8 fase38_c_manifest_discovery = **356 axon-frontend tests green**; 2077 axon-rs lib + 48 store integration targets (fase35_l 17 + fase37x_a 5 + fase35_fuzz 6 + fase37_d 9 + fase37_g 3 + fase37x_i_pgbouncer 4 + fase37x_i_property_fuzz 4) all green; 74 Python contract/cli/golden + drift gates green. **Total ~2555 tests, zero regressions, zero new crate deps, zero Python work**. |
| **38.e** | **`axon check` column-type proof for `persist`/`mutate`/`purge` (D2 second half).** Extend `store_column_proof.rs` with `check_persist(fields, declared_columns)`, `check_mutate(set_fields, where_filter, declared_columns)`, `check_purge(where_filter, declared_columns)`. Three new error codes: `axon-T803` (`persist` omits a NOT-NULL column with no default), `axon-T804` (field-name typo in `persist`/`mutate` with suggestions). `check_mutate` runs `check_filter` on the `where:` clause + the new field-block proof. `check_purge` is `check_filter` only (no fields). 25+ unit tests covering all 4 store ops × all 9 proof error codes × the 15-type catalog. Anchor §3 INVERTED in place. | OSS | D2, D6, D5 | ✅ SHIPPED — `store_column_proof.rs` extended with `check_persist_fields(fields, columns, flow_params, op_loc)` + `check_mutate_fields(...)` + `check_field_block_columns` (shared helper) + `classify_field_value(raw) -> WhereValue` (proof-purpose field-value lexical classifier — recognises `${name}`/`$name` as `BoundParam`, `'literal'` strings as Text, `42`/`-7` as Int, `3.14` as Float, `true`/`false` as Bool, `null` as NullKeyword, everything else as Text — the conservative classification because the runtime sends multi-interpolation values as text strings). **Two new error codes**: `ProofErrorCode::T803NotNullOmitted` (a `persist` block populates SOME columns but omits a NOT-NULL column without a default; uses an empty-fields = blockless = SKIP rule so D5 absolute is preserved for adopters using v1.30.0 blockless `persist <store>`) + `ProofErrorCode::T804UnknownField` (field-block column doesn't exist in declared schema, with Fase 28 Levenshtein "Did you mean X?" hint). **T802 reused** for field-block value-type mismatches (literal-shape vs column type + bound-param-axon-type vs column type, mirroring 38.d's compat matrix) AND for `NULL → NOT-NULL column` violations. **`DeclaredColumn` extended** with `has_default: bool` (true if `default_value` non-empty OR `auto_increment` is set) + `primary_key: bool` (informational) so the T803 NOT-NULL omission detection knows which columns can be safely omitted from a persist (auto_increment / explicit-default columns are skippable). `ColumnSet::from_inline_columns` and `from_manifest_store` updated to thread the new flags. **`check_mutate_fields` deliberately does NOT emit T803** — UPDATE preserves existing row values for omitted columns; only persist's INSERT can fail NOT-NULL. **Type-checker integration**: new `run_38e_persist_proof` invoked on `FlowStep::Persist` + new `run_38e_mutate_proof` invoked on `FlowStep::Mutate` (alongside the existing `run_38d_where_proof` for mutate's where: clause). `FlowStep::Purge` already covered by 38.d's where:-only proof. **`check_mutate_fields` test (`mutate_does_not_emit_t803_for_omitted_not_null_columns`)** pins the asymmetry between persist and mutate. **28 new unit tests in `store_column_proof::tests`** (70 total — +28 from 38.e): `classify_field_value` happy paths (braced/bare bound-param, int, float, bool case-insensitive, null, multi-interpolation → Text, empty → Text) + T803 NOT-NULL omission (with/without default + auto_increment exemption + nullable column + multiple omissions + blockless skipped) + T804 unknown field (with Levenshtein hint + out-of-distance no-suggestion) + T802 field-block (Int → Text rejection + Bool param → Int rejection + Bool param → Bool passes + NULL → NOT-NULL rejected + NULL → nullable passes) + happy-path persist + D5 blockless persist + mutate-no-T803 + mutate-T804 typo + mutate-T802 value mismatch + happy-path mutate + D5 blockless mutate + 15-type catalog smoke (String param accepted against every catalog type, mirroring the D4 runtime fallback). **Anchor §3 INVERTED in place**: was "axon check passes a persist field-name typo"; now asserts `axon-T804` fires + the field name + the Levenshtein suggestion are surfaced. Anchor file post-cleanup: 5 tests (§1+§2 inverted by 38.d; §3 inverted by 38.e; §4 38.b regression guard; §5 38.h observability of absent CLI). **Verification — zero regressions**: 355 axon-frontend lib (+28 from store_column_proof::tests, 327 → 355) + 4 fase28 + 5 fase38_a + 11+1 fase38_b + 8 fase38_c = **384 axon-frontend tests green**; axon-rs lib 2077 + 48 store integration untouched; cross-stack 7 Python drift + 74 Python contract sweep unchanged. **Total ~2589 tests, zero regressions, zero new crate deps, zero Python work**. D5 absolute upheld: blockless persist/mutate skip the proof verbatim (the v1.30.0 user-bindings fallback). Forms (b)/(c) compile-time proof still deferred to 38.h/38.j honestly. |
| **38.f** | **Per-tenant `schema: env:VAR` resolution (D3).** The deploy-time resolver in `axon-rs/src/store/registry.rs` extends 37.x's `verify_postgres_schemas` to honor a declared `schema: env:VAR` — the env var resolves to a SQL schema name (the namespace), the manifest is looked up by `<namespace>.<store_name>`, the proof runs against the resolved columns. A missing env var → `axon-T806` deploy failure (never a silent fallback). The resolved namespace is stamped into the `application_name` (Gap-3 inheritance: `axon-store/<store>/<namespace>`) so DBA tooling sees the tenant. 8+ deploy-handler tests + a 3-tenant CI lane (D7 lane 3). | OSS | D3, D6 | ✅ SHIPPED — `axon-rs/src/store/postgres_backend.rs` + `axon-rs/src/store/registry.rs` extended (~+450 LOC, 100% Rust, zero new crate deps). **Two new StoreError variants**: `MissingPerTenantSchemaEnv { store, var }` → `axon-T806`; `DeclaredVsLiveDrift { store, drift }` → `axon-T807`. Both Display arms carry the slug + a named, actionable remedy (T806 names env-var + alternative declaration forms; T807 names `axon store introspect <store>` + migration + declaration fix). **`application_name_for_with_namespace(store, Option<&str>)`** new helper — preserves the v1.36.3 `axon-store/<store>` shape when namespace is None, produces `axon-store/<store>/<namespace>` when Some; caps at NAMEDATALEN-1 on a UTF-8 char boundary. The old `application_name_for` delegates (byte-identical for non-namespace stores). **New `PostgresStoreBackend::connect_named_with_namespace`** constructor — `connect_named` delegates with `None` (D5 absolute). **`verify_postgres_schemas_with_manifest(&self, manifest: Option<&Manifest>) -> SchemaVerifyReport`** is the new core; the existing `verify_postgres_schemas()` delegates with `None` so the v1.37.0 deploy handler keeps its v1.37.0 behavior verbatim. The with-manifest variant: (1) reads `column_schema` from each pg store's IR spec; (2) for form (c) EnvVar resolves the env var FIRST — missing/empty → T806 fatal, returns BEFORE any DB work; (3) for form (c), restamps the pool-cache entry with `connect_named_with_namespace(..., Some(&namespace))` so every runtime session carries the tenant in its `application_name` (Gap-3 inheritance); (4) calls `warm_schema(name)` (37.x infrastructure); (5) when columns are declared, calls `verify_declared_columns` to compare declared columns vs live introspection — a drift surfaces T807 fatal in `report.missing`. **`declared_columns_for`** resolves form (a) inline to a BTreeMap directly; form (b) ManifestRef via `manifest.lookup(qualified_name)` (missing entry → named error citing available entries); form (c) EnvVar via `<namespace>.<store_name>` first then first-match heuristic `*.<store_name>` (mirrors 38.d's `load_columns_for_schema`). **`pg_udt_matches_catalog_type`** is the case-insensitive declared-vs-live compat predicate per the v1.30.0 PgTypeClass mapping — Text accepts text/varchar/bpchar/name; Int accepts int4/integer; BigInt accepts int8/bigint; etc. **Pool-cache restamping** in `restamp_backend_with_namespace` is idempotent — verifying twice with the same namespace is a no-op; same DSN with a different namespace produces a new namespace-stamped pool that replaces the old one in the cache. **Honest scope deferral**: NOT-NULL parity is NOT yet proven by T807 (37.x's `introspect_conn` query doesn't capture `attnotnull`); the runtime catches NOT-NULL drift via SQLSTATE 23502 at the first failing persist (defense-in-depth retained); 38.f.2 follow-on will extend the introspection. **Forms (b)/(c) with NO manifest in scope at deploy** silently fall through to 37.x existence-only verification (not T807) — the v1.37.0 deploy handler today passes `None`, so this is the deploy-time floor until 38.h/38.j plumb the manifest. **Deploy-handler integration**: `axon_server.rs` line 1734's call stays at `verify_postgres_schemas()` (delegates to with-manifest `None`) — wiring the manifest in lands in 38.h's CLI + 38.j's CI lane. **16 new unit tests** in `store::registry::tests`: T806 missing-env-var-fails-deploy, T806 empty-string-env-var, 3-tenant restamping independence, application_name stamping (4 cases incl. boundary + truncation), pg_udt_matches_catalog_type (text aliases + cross-class rejects), verify_declared_columns no-schema-skips, declared_columns_for inline/inline-unknown-type-error/manifest-ref-no-manifest/env-var-no-manifest/manifest-ref-resolves/env-var-first-match/manifest-ref-missing-entry, T806+T807 Display slugs+remedies. **45 registry tests total** (16 new + 29 prior). **Verification — zero regressions**: 2093 axon-rs lib (+16 from new 38.f tests) + 17 fase35_l + 5 fase37x_a + 6 fase35_fuzz + 9 fase37_d + 3 fase37_g + 4 fase37x_i_pgbouncer + 4 fase37x_i_property_fuzz = **2141 axon-rs tests green**; 355 axon-frontend lib + 5 anchor + 11+1 fase38_b + 8 fase38_c = 380 axon-frontend tests untouched; Python contract sweep + Python drift gate untouched. **~2521 total Rust tests green; zero regressions, zero new crate deps, zero Python work**. D5 absolute upheld: undeclared schema + form (b)/(c) without manifest both fall through to v1.37.0 behavior. |
| **38.g** | **Suggest dictionary integration — Levenshtein hints.** The Fase 28 Levenshtein "Did you mean X?" infrastructure is already in `axon-frontend/src/suggest.rs`. 38.g wires the column-name proof through it: the suggestion source is the declared schema's columns (a stable, closed set per store — not the Fase 29 vertical dictionary, which is for natural-language top-level keywords). Max 3 candidates within edit distance ≤2. Composite suggestions when a column AND a type both mismatch ("Did you mean column `tenant_id` (Uuid)?"). The vertical-aware dictionary path (Fase 29) layers ON TOP — e.g. an enterprise HIPAA vertical can register `medical_record_number` as a high-frequency synonym for `mrn`, which Fase 28's mechanism already supports. 12+ tests covering the Levenshtein boundary cases + multi-candidate ordering + composite-suggestion shape. | OSS | D6 | ✅ SHIPPED — `store_column_proof.rs` extended with three composite-suggestion helpers (~+170 LOC; 100% Rust, zero new crate deps, zero Python work). **`format_column_list(&ColumnSet) -> String`** renders the declared schema as `{name: Type, name: Type, …}` alphabetically (deterministic per the underlying BTreeMap); replaces the bare-names rendering in T801/T804 error bodies so adopters see EACH column's type at a glance. **`suggest_columns_composite(unknown, &ColumnSet) -> String`** wraps Fase 28's `smart_suggest::suggest` with composite phrasing: 1 match → `Did you mean column \`X\` (Type)?`; 2 → `Did you mean column \`A\` (T1) or \`B\` (T2)?`; 3 → `Did you mean column \`A\` (T1), \`B\` (T2), or \`C\` (T3)?` (Oxford-comma style); empty when no candidate within `MAX_DISTANCE = 2`. Hard-caps at `MAX_RESULTS = 3` (Fase 28 D3 inheritance). **`suggest_type_compatible_columns_for_literal(LiteralKind, &ColumnSet, excluded_column)`** + **`suggest_type_compatible_columns_for_param(axon_type, &ColumnSet, excluded_column)`** are the T802 tactical-hint pair — when a literal or bound-param type clashes with the declared column, scan the rest of the schema for columns whose type WOULD accept the value, render up to `MAX_COMPAT_SUGGESTIONS = 3` as `Compatible Int-class columns in this schema: \`account_id\` (Int), \`created_at\` (Int).` The failing column itself is excluded — no "did you mean the column that just failed" noise. **T801 + T804 message paths** now use `format_column_list` for the declared-columns body AND `suggest_columns_composite` for the suggestion — both adopter-facing surfaces upgraded uniformly. **T802 paths** (literal + bound-param in BOTH `check_predicate` and `check_field_block_columns`) now append the compatible-columns hint when one exists. **LIKE-vs-non-text T802 deliberately keeps its v38.d shape** — LIKE wants a Text-class column, not a Text VALUE; surfacing "compatible columns" for a LIKE operator would mislead. **Vertical-aware Fase 29 extensibility documented**: an enterprise tenant that registers `medical_record_number` as a synonym for `mrn` passes the augmented column-name list to `suggest_columns_composite` — Fase 28's `smart_suggest::suggest` already accepts any candidate slice (D3); no new infrastructure required in 38.g for the synonym path. **15 new unit tests in store_column_proof::tests (85 total — +15 from 38.g)**: `format_column_list` (alphabetic + name:type + empty-schema), `suggest_columns_composite` (single match + two-match `or` separator + three-match Oxford comma + no-candidate-empty + caps-at-three), T801 composite message shape, T804 composite message shape, T802 literal mismatch surfaces compatible columns, T802 omits the failing column from the suggestion, T802 bound-param mismatch surfaces type-compatible columns, T802 field-block literal also surfaces compatible columns, T802 caps at MAX_COMPAT_SUGGESTIONS, T802-LIKE-vs-non-text does NOT surface a misleading compat hint. **Verification — zero regressions**: 370 axon-frontend lib (+15 from 38.g) + 4 fase28 + 5 fase38_a + 11+1 fase38_b + 8 fase38_c = **399 axon-frontend tests green**; axon-rs untouched (2141 + 380 axon-frontend = ~2541 surface unchanged structurally from 38.f — adding the 15 new tests brings 38.g's total contribution to ~2556 Rust tests). Zero new crate deps. Zero Python work. D5 absolute upheld: the helpers only surface when called from T801/T802/T804, all of which gate on a declared schema. |
| **38.h** | **`axon store introspect` CLI — manifest export (D10).** New subcommand `axon store introspect <store_name> [<store_name>...] [--manifest=<path>] [--json] [--diff]`. Opens the store's resolved DSN (37.x infrastructure), introspects via `pg_catalog` (37.x's `introspect_conn`), maps every Postgres type to D1's 15-type catalog. An unmappable Postgres type is an honest `UnsupportedColumnType` — the column is OMITTED from the manifest with a `# omitted: enum 'tier_enum'` comment, never silently lossily mapped (`enum` ≠ `Text`, even though they look alike at the wire). Writes canonical YAML by default, JSON with `--json`, `--diff` mode emits only the columns that changed vs. an existing manifest. `*` glob support — `axon store introspect *` introspects every declared `postgresql` store. The 37.x D9 schema cache speeds repeat calls. Anchor §5 INVERTED in place. 20+ unit tests + 5 integration tests (real Postgres). | OSS | D10, D6 | ✅ SHIPPED — first runtime CONSUMER of the §38.c manifest format, split between two crates per the existing pure/impure architectural seam (axon-frontend = no runtime deps; axon-rs = sqlx + Tokio). **New `axon-frontend/src/store_introspect.rs`** (~740 LOC pure Rust, zero new crate deps): `udt_to_canonical_type(pg_udt) -> Option<StoreColumnType>` reverse-mapping mirroring `pg_udt_matches_catalog_type` from 38.f; `IntrospectionRow { column_name, pg_udt, not_null, primary_key, unique, default_expression }` input shape; `detect_auto_increment(default_expression) -> bool` (case-insensitive `nextval(` substring heuristic); `OmittedColumn { name, pg_udt, reason }` with `as_comment_line()` rendering `# omitted: column \`X\` (pg type \`Y\`) — <reason>`; `build_manifest_store(rows) -> (ManifestStore, Vec<OmittedColumn>)` pure builder (auto_increment columns DROP the `nextval(...)` expression since it references a sequence — sequence names are adopter-private); `ColumnDelta { Added, Removed, TypeChanged, ConstraintChanged }` + `ManifestDiff { per_store, added_stores, removed_stores }` + `manifest_diff(old, new) -> ManifestDiff` (type-change subsumes constraint flips for the same column to avoid noise; each constraint facet — primary_key / not_null / unique / auto_increment / default_value — surfaces as its own delta when it independently flips); `format_manifest_diff(diff) -> String` renders human-readable summary with `+ column`, `- column`, `~ column type:`, `~ column <facet>:` lines. **26 pure unit tests** covering the full closed catalog (every pg_udt → canonical mapping + case-insensitivity + non-catalog rejection), auto_increment heuristic (positive + negative cases), build_manifest_store (catalog mapping + omission tracking + constraint pass-through + serial/nextval handling + static-default preservation + alphabetic ordering + empty), OmittedColumn rendering, manifest_diff (empty + added store + removed store + added column + removed column + type-change + type-change-subsumes-constraint + every-facet-independent + format_manifest_diff text + empty-diff). **New `axon-rs/src/store/introspect_cli.rs`** (~360 LOC + 3 unit tests): `introspect_store(connection, store_name)` opens a single-use Postgres connection with `application_name` stamped `axon-store/<store>/introspect` (38.f Gap-3 inheritance), resolves the table's schema via `to_regclass` (search-path-correct) with a search-path-INDEPENDENT cross-schema scan fallback, runs a DEEP `pg_catalog` introspection query joining `pg_class`⋈`pg_namespace`⋈`pg_attribute`⋈`pg_type` plus subqueries for primary-key membership (`pg_constraint.contype = 'p'`) + single-column unique membership (`contype = 'u'` AND `conkey = ARRAY[attnum]`) + default expression (`pg_get_expr(adbin, adrelid)`), decodes via `decode_introspection_row`, hands to the pure builder, packages the result as a one-store `Manifest` keyed by `<resolved-schema>.<store_name>` with `refresh_content_hash()`. `introspect_stores(connection, &[store_names])` merges multi-store introspection into one Manifest with one content hash. `INTROSPECT_NAMESPACE = "introspect"` pinned for DBA-observable contract stability. `render_introspection_output(&manifest, &omissions)` emits canonical JSON manifest + tail of `# omitted: …` comment lines (no trailing newline when no omissions). The 37.x runtime `introspect_conn` is deliberately LEFT INTACT — the deep query is a separate code path so the runtime hot path is unaffected (the 37.x D9 schema cache still owns the runtime hot path). `application_name` for CLI sessions is distinct from runtime sessions AND from per-tenant 38.f sessions — three observable contexts in `pg_stat_activity`. **New CLI subcommand**: `axon store introspect <store_names>... --connection <dsn> [--output <path>] [--diff <existing.json>] [--format json]`. Stub `--format yaml` fails fast pointing at the 38.c.2 follow-on (`yaml-manifest` Cargo feature). `--diff` reads existing manifest, computes `manifest_diff`, prints human-readable summary; emits "manifest is up to date" when no drift. `mask_dsn_pub` public alias added to `postgres_backend.rs` so the CLI renders error messages with masked credentials. **6 integration tests** (PG-gated, graceful skip per `AXON_TEST_DATABASE_URL` discipline, in `axon-rs/tests/fase38_h_introspect_integration.rs`): t1 happy-path 15-type-catalog (10 columns covering Uuid/Text/Int/BigInt/Numeric/Bool/Timestamptz/Date/Jsonb/Bytea, primary_key + not_null survive, content-hash verifies); t2 omits unmappable types (`POINT` + `INET` → 2 omissions with reason naming "closed type catalog"; the `id UUID` survives — D10/D6 honest-omission guarantee); t3 constraints round-trip (PRIMARY KEY + NOT NULL + UNIQUE + DEFAULT 'standard' all flow through); t4 auto_increment heuristic recognises SERIAL (default `nextval(...)` → `auto_increment: true` + the expression DROPPED); t5 diff mode flags column-type changes (CREATE int → ALTER int8 → diff names `column \`id\` type: Int → BigInt`); t6 multi-store introspect merges into one manifest (bonus — exercises `introspect_stores` aggregation). **Anchor §5 INVERTED in place** in `fase38_a_typed_store_diagnostic.rs`: was "absent module" narrative-pin; NOW a regression guard exercising the full public API surface (`udt_to_canonical_type` + `detect_auto_increment` + `build_manifest_store` + `OmittedColumn::as_comment_line` + `manifest_diff` + `format_manifest_diff` + 5 data types). All 5 pre-38 anchors §1–§5 are now regression guards. **Verification — zero regressions**: 393 axon-frontend lib (+23 from new `store_introspect::tests`, was 370) + 4 fase28 + 5 fase38_a + 11+1 fase38_b + 8 fase38_c = **422 axon-frontend tests green**; 2096 axon-rs lib (+3 from `introspect_cli::tests`, was 2093) + 48 prior store integration + 6 new fase38_h_introspect = **2150 axon-rs tests green**; ~2572 total Rust tests green. Zero new crate deps. Zero Python work since 38.b. D5 absolute upheld — the introspection CLI is a new surface; no existing call-site changes. |
| **38.i** | **Integration + property/fuzz tests.** Two new test targets. (a) `axon-frontend/tests/fase38_i_integration.rs` — end-to-end: parse a multi-store source with mixed inline/manifest/env-var declarations, run `axon check` against a curated 5-tenant manifest, assert every D2 error code fires for the right input + every good-case passes. (b) `axon-frontend/tests/fase38_i_property_fuzz.rs` — ~6 000 deterministic LCG iters across three surfaces: Surface A (`parse_manifest` totality — random YAML bodies including malformed; never panic, deterministic error mapping); Surface B (`StoreColumnProof::check_filter` totality — random parsed filter ASTs × random declared schemas; never panic, every verdict is a deterministic combination of declared columns + filter columns + parameter types); Surface C (`canonical_serialize` is a fixed point — serializing a parsed manifest then re-parsing yields a structurally-equal manifest; the hash matches its content). | OSS | D7, D5 | ✅ SHIPPED — three new files exercise the full Fase 38 contract surface end-to-end. **NEW fixture**: `tests/fixtures/fase38_i/curated_manifest.axon-schema.json` — a 5-store curated manifest mirroring real adopter shapes (`public.tenants`, `public.events`, `audit.log`, `tenant_alpha.usage`, `tenant_beta.usage` — last two same-shape per-tenant, exercising the first-match heuristic policy from 38.d/38.f). **NEW `axon-frontend/tests/fase38_i_integration.rs` (21 tests)** organized in six sections — A: curated-manifest 5-store parse + per-tenant-shape-identity round-trip + content-hash round-trip; B: mixed inline/manifest_ref/env_var multi-store source parses with the right IR variants per store + axon-check passes when flow touches only the inline store + axon-check flags inline-proof failure WITHOUT firing on the manifest_ref/env_var stores (honest-scope discipline); C: every D2 error code (T801/T802/T803/T804) fires for its corresponding adopter-flavor trigger + composite Levenshtein suggestion `column \`X\` (Type)` is present + T802 literal mismatch surfaces compat hint when alternative exists + T802 with no alternative suppresses the hint cleanly + happy-path persist + happy-path retrieve with canonical Uuid string literal + happy-path purge with where-only; D: D5 absolute upheld — undeclared schema skips proof verbatim + blockless persist against inline schema skips proof; E: Levenshtein boundary integration — distance-2 typo surfaces composite suggestion + out-of-distance typo lists columns without a guess; F: 38.h public-API integration smoke (every exported symbol consumable from a downstream consumer). **NEW `axon-frontend/tests/fase38_i_property_fuzz.rs` (5 tests, ~6 200 deterministic LCG iters)** — Surface A (`fuzz_surface_a_manifest_parse_is_total_and_deterministic`, 2 000 iters): random JSON inputs from curated fragments + programmatic synthesis + random-ASCII garbage; invariants A.1 totality, A.2 closed-error-catalog (no `DuplicateStore` from single-file parse), A.3 determinism, A.4 canonical-form round-trip preserves equality. Surface B (`fuzz_surface_b_check_filter_is_total_and_deterministic`, 2 000 iters + a 200-iter `empty_where_emits_zero_errors` companion): random `where:` × random ColumnSet × random FlowParamTypes; invariants B.1 totality, B.2 closed `{T801, T802}` namespace (T803/T804 field-only, T805 manifest-only confirmed never surface from check_filter), B.3 determinism, B.4 empty/malformed `where:` emits zero errors. Surface C (`fuzz_surface_c_canonical_serialize_is_a_fixed_point`, 2 000 iters + 400-iter `hash_differs_when_content_differs` companion): random Manifest construction; invariants C.1 `parse(canonical(m)) == m`, C.2 canonical-form byte-fixed-point under repeated serialisation, C.3 content-hash stable across round-trip, C.4 `verify_content_hash` holds after `refresh_content_hash` + canonical-with-hash round-trip. **Verification — zero regressions**: 393 axon-frontend lib + 4 fase28 + 5 fase38_a + 11+1 fase38_b + 8 fase38_c + **21 fase38_i_integration (NEW) + 5 fase38_i_property_fuzz (NEW)** = **448 axon-frontend tests green** (+26 from 38.i; was 422 at end of 38.h); axon-rs (untouched by 38.i): 2096 lib + 54 store integration = 2150 tests green; ~2598 total Rust tests; zero new crate deps; zero Python work since 38.b; D7 cycle-fuzz coverage anchored. |
| **38.j** | **CI lane + adopter docs.** `.github/workflows/fase_38_typed_store_schema.yml` with 5 parallel lanes (per D7): pure-column-proof, manifest-roundtrip, per-tenant resolution, declared-vs-live drift, D5 zero-regression. Adopter docs: `docs/ADOPTER_AXONSTORE.md` new top-level §17 "The Compile-Time-Typed Store Schema (v1.38.0)" with the 5 recipes (inline column block / manifest reference / per-tenant env-var / introspect-then-commit / migration-induced drift detection); `docs/MIGRATION_v1.38.md` with 6 scenarios A-F (no `schema:` — no change / opt into inline / opt into manifest / opt into per-tenant / `axon check` now reports column typos / deploy now fails on declared-vs-live drift); a new `docs/ADOPTER_TYPED_STORE.md` deep-dive on the 5-pillar story (the COLUMN pillar joining Epistemic + Audit + Streaming + Capability + **TypedColumn**); the `AXON_GAP_store_typed_columns_resolution.md` from 37.x extended with a closing paragraph naming the compile-time half delivered here. | OSS | D7, D6 | ✅ SHIPPED 2026-05-20 — the load-bearing 38.j wire that ACTIVATES T807 in production lands here. **1) `axon serve --schemas-dir <path>` flag + deploy_handler manifest plumbing**: `Commands::Serve` enum in `axon-rs/src/main.rs` extended with `schemas_dir: Option<String>` (CLI flag + `AXON_SCHEMAS_DIR` env var; CLI wins; empty-string-collapses-to-None; mirrors the 31.f / 36.g resolution ladder); `ServerConfig.schemas_dir: Option<String>` new field in `axon-rs/src/axon_server.rs` (full doc-comment with three converging surfaces + D5 absolute disclaimer); `deploy_handler` extended to call `load_and_merge_manifests` on every deploy when the field is Some(_) and then `verify_postgres_schemas_with_manifest(Some(&m))` (38.f's API), so T807 fires the moment a deploy's declared columns disagree with live. Manifest-load failure surfaces as structured 200-OK with `success: false`, `phase: "store_schema_manifest_load"`, `d_letter: "D3+D8"`, and the operator-supplied `schemas_dir` echoed back (operator can pinpoint the offending dir without grep). **53+2 test files** under `axon-rs/tests/` updated with `schemas_dir: None` field on every existing `ServerConfig` literal (PowerShell sweep for the 53 literal-`None` form + two variable-bind cases for fase36_g / fase36_j). **2) New `axon-rs/tests/fase38_j_schemas_dir_plumbing.rs`** (6 cases, ~300 LOC, infra-free — no Postgres): §1 D5 — no schemas_dir = pre-38 behavior verbatim; §2 empty dir = clean no-op; §3 non-existent dir = clean no-op (`load_and_merge_manifests` is total); §4 T805 hash mismatch returns the structured shape with `phase` + `d_letter` + `schemas_dir` echo; §5 duplicate-store across two files returns the same shape with the offending store named; §6 well-formed manifest with verified `sha256:` content_hash threads clean. ALL 6 cases green. **3) New `.github/workflows/fase_38_typed_store_schema.yml`** (5 parallel lanes + summary): lane 1 pure-column-proof (`store_column_proof::` + `fase38_a_typed_store_diagnostic` + `store_introspect::`); lane 2 manifest-roundtrip (`store_schema_manifest::` + `fase38_i_integration` + `fase38_i_property_fuzz` ~6 200 LCG iters); lane 3 per-tenant-resolution (`store::registry::` + `store::postgres_backend::`); lane 4 declared-vs-live-drift (the new `fase38_j_schemas_dir_plumbing` 6-case pack); lane 5 d5-zero-regression (`fase37x_i_property_fuzz` ~7 500 + `fase35_fuzz` ~18 000 LCG iters + `fase32_routes_drift` + `fase32_auth_scope` + `fase32_replay_binding` + `fase33_c_live_event_forwarding`). **4) `docs/MIGRATION_v1.38.md` (~290 LOC, 6 scenarios A-F)**: A no axonstore — no change; B `axonstore` without `--schemas-dir` — byte-identical D5; C adopt form (a) inline columns; D adopt form (b) manifest reference (introspect-then-commit workflow); E adopt form (c) per-tenant env-var (multi-tenant SaaS); F troubleshooting `phase: store_schema_manifest_load`. Plus CI-integration section + "what stays the same" disclaimer. **5) `docs/ADOPTER_AXONSTORE.md` §17 (~280 LOC)**: 5 recipes — §17.1 form (a) inline with end-to-end T801/T803/T804 surfacing; §17.2 form (b) introspect → reference → boot 3-step; §17.3 form (c) per-tenant with first-match + Gap-3 application_name + T806; §17.4 introspect CLI use cases (first adoption / CI contract gate / drift forensics); §17.5 wiring `--schemas-dir` in systemd/Kubernetes/Dockerfile with the failure-mode table. Table of contents updated. **6) NEW `docs/ADOPTER_TYPED_STORE.md` (~420 LOC)** — architectural companion: ASCII layer diagram; per-pillar section (I Epistemic / II Audit-chained / III Streaming / IV Capability / V TypedColumn) with failure-mode-closed + surface + why-it-lives-on-axonstore + composes-with-Pillar-V; the Pooler-Coherent Store Contract substrate; adoption ladder; per-pillar CI gate; closing "what each pillar costs" + founder principle quote. **7) `docs/fase/AXON_GAP_store_typed_columns_resolution.md`** extended with a closing "Compile-time half (v1.38.0)" section: 3-row layer table (compile-time / deploy-time / runtime — v1.37.x said vs v1.38.0 adds); adopter-visible surfaces link table; closing line "*Compile-time half closed 2026-05-20.*" **Verification — zero regressions**: 2096 axon-rs lib + 6 fase38_j_schemas_dir_plumbing + 6 fase35_fuzz + 11 fase32_routes_drift + 10 fase33_c_live_event_forwarding + 6 fase37x_i_property_fuzz green; 21 fase38_i_integration + 5 fase38_i_property_fuzz + 5 fase38_a_typed_store_diagnostic green; full axon-frontend lib+tests sweep zero failures (~448 axon-frontend tests). **Zero new crate deps. Zero Python work**. T807 now fires in production the moment a `POST /v1/deploy` arrives whose declared columns disagree with live introspection — Fase 38 functionally complete. |
| **38.k** | **Coordinated release** — axon-lang **v1.38.0** cross-stack (`bump-my-version` minor 1.37.0→1.38.0; crates.io + PyPI + GitHub Release + binaries). `axon-frontend` BUMPS to **0.19.0** — Fase 38 IS a frontend AST + type-checker change (the first since v1.37.0 left axon-frontend at 0.18.0). axon-enterprise catch-up — **v1.29.0** (image bump, dep pin `>=1.37.0`→`>=1.38.0`; same lean-catch-up shape as every minor since v1.9.0). | SPLIT | — | ✅ SHIPPED 2026-05-20 — coordinated cross-stack release. **axon-lang v1.38.0**: `bump-my-version` minor 1.37.0 → 1.38.0 (6 file anchors: `pyproject.toml`, `axon/__init__.py`, `axon-rs/Cargo.toml`, `tests/test_cli_mvp_smoke.py` ×2, `tests/test_frontend_contract_golden.py`). **axon-frontend 0.18.0 → 0.19.0** with description-prefix updated to lead with the Fase 38 Declared & Compile-Time-Typed Store Schema surface (the 3 declaration forms + the 6 error codes T801–T805 + T807). **`axon-rs` dep pin** `axon-frontend = "=0.18.0"` → `"=0.19.0"`. Cargo.lock files auto-regenerated via `cargo build` (axon-frontend warmed in 9.90s; axon-rs in 2m 45s — zero compile errors, only pre-existing dead-code warnings). 8 files, 13/13 line changes — identical footprint to v1.36.0's coordinated bump. Single commit `release(v1.38.0): coordinated cross-stack bump` → origin master → tags `v1.38.0` + `rust-v1.38.0` + `axon-frontend-v0.19.0` pushed → `Rust Release Binaries` workflow fires on `rust-v*` → `cargo publish axon-frontend 0.19.0` THEN `cargo publish axon-lang 1.38.0` (lockstep ordering — the dep pin requires frontend on the index first) → GitHub Release v1.38.0 created with content-first notes (the 5-pillar story + the 3 declaration forms + the 6 error codes + the introspect CLI). PyPI: `Publish to PyPI` workflow fires on `release: published`; recovery path = draft-toggle if the trigger drops (project_axon_lang_publish_trigger_quirk.md). axon-enterprise **v1.29.0** catch-up: branch off `enterprise/master`, two-file bump (`pyproject.toml` version + `axon_enterprise/__init__.py` `__version__` + `axon-lang>=1.37.0` → `>=1.38.0` dep pin), regular-merge PR, refspec-mapped tag `enterprise/v1.29.0:refs/tags/v1.29.0`, GitHub Release v1.29.0 published — Fase 38's typed-column proof becomes available cross-stack to every enterprise adopter as a transitive dep advance, NO enterprise-code change. Two-phase Wait step (axon-enterprise PR #14 commit `9d7fe12`) prevents the GitHub CDN race. Fase 38 CYCLE CLOSED — D1–D10 ratified, all 11 sub-fases (38.a–38.k) shipped, the COLUMN pillar joins Epistemic + Audit + Streaming + Capability as the fifth load-bearing pillar of the cognitive data plane. |
