...
status: ✅ CLOSED 2026-05-17 — D1–D6 RATIFIED (founder bloque, 2026-05-17). ALL sub-fases 36.x.a–36.x.h + 36.x.e.2 ✅ SHIPPED. axon-lang v1.35.0 live cross-stack (crates.io axon-frontend 0.17.0 + axon-lang 1.35.0 + PyPI 1.35.0 + GitHub Release v1.35.0 with 5-platform binaries) + axon-enterprise v1.26.0 catch-up live (PR #31 merged, tag v1.26.0, Docker→ECR, GitHub Release). Triggered by the founder's post-v1.34.0 hypothesis (a streaming `axonendpoint` only executes pure-`step` flows, not the real agent pattern that mixes store-ops with a step), investigated 2026-05-17: the path DOES dispatch mixed flows, but the investigation surfaced a real double-terminator wire bug, zero test coverage, and a structural blocker (no in-memory-declarable store → mixed flows cannot run or be tested without a live Postgres). Target axon-lang v1.35.0. Rust-canonical.
...
# ▶ 4. D-letters (D1–D6 — RATIFIED founder bloque 2026-05-17)
...
| **36.x.e.2** | Streaming-tool path observability parity — `run_step_streaming_tool` surfaces the enforcement summary like the LLM-side path. Resolves the pre-existing cluster 36.x.e discovered. | OSS | D4 | ✅ SHIPPED — root cause: Fase 36.i wired the tool registry, re-routing `step { apply: <stream-tool> }` from the LLM-side `run_pure_shape` path to `run_step_streaming_tool`, which never wrote `ctx.enforcement_summaries` — so `enforcement_summary` was empty on `axon.complete` for an `apply:`-streaming-tool step. v1.34.0 shipped it (the double-terminator masked it; 36.x.c unmasked it). Fix: `run_step_streaming_tool` now maps its `ToolStreamSummary` → `EnforcementSummaryWire` and writes `ctx.enforcement_summaries` (keyed by step name) when the applied tool declared a `<stream:<policy>>` effect — parity with `run_pure_shape::drain_through_enforcer`. The 3 cluster test files were reconciled to the streaming-tool path's actual (correct) shape: `fase33x_d_enforcer_activation` + `fase33z_k_h_metadata_population` count assertions made robust (the stub stream emits >1 chunk; assert ≥1 produced + all delivered); `fase34_a_unified_stream_handler_diagnostic` §2/§3 inverted from `== 1` to `> 1` — the v1.28.0 comment had pre-announced "POST-34 inverts this". `fase33x_d` 11/11, `fase33z_k_h` 12/12, `fase34_a` 7/7; 156 streaming regression tests green; zero regressions. |
| **36.x.f** | Exactly-one-terminator property/fuzz pass — a deterministic property test that `run_streaming_via_dispatcher` emits exactly one terminator over arbitrary flow shapes (pure-step, mixed, erroring, cancelled, empty, multi-store). | OSS | D1, D6 | ✅ SHIPPED — `axon-rs/tests/fase36x_f_terminator_fuzz.rs` (2 tests): 600 deterministic LCG iterations drive `run_streaming_via_dispatcher` directly over 7 generated shape archetypes — pure-step (1–3 steps), mixed (in_memory store + retrieve + step + persist), `apply:`-streaming-tool, empty, sqlite-error (§2.5 registry-build failure), garbage source (§2 compile-error), flow-not-found (§3), each optionally pre-cancelled (~1/8). Invariants: `terminator_count <= 1` ALWAYS (the core D1 — never a double terminator); `== 1` for any non-cancelled run; the terminator is the FINAL event with no earlier duplicate; `FlowStart` leads. + a 4-archetype deterministic anchor independent of the LCG walk. Test-only sub-fase — no production change; 2/2 green. |
| **36.x.g** | CI lane (`fase_36x_mixed_flow_streaming.yml`) + adopter docs — `docs/ADOPTER_AXONSTORE.md` streaming section + the canonical agent-pattern recipe + `docs/MIGRATION_v1.35.md`. | OSS | D6, D5 | ✅ SHIPPED — new `.github/workflows/fase_36x_mixed_flow_streaming.yml` (4 parallel lanes: `rust-frontend` = `in_memory` declarable backend `fase36x_b`; `rust-runtime` = the full mixed-flow surface end-to-end `fase36x_a/b/d/e`; `fuzz-d1` = the 600-iter exactly-one-terminator property pass `fase36x_f`; `regression` = D5 — Fase 33/34 streaming surfaces 36.x touches: `fase33_b`/`fase33_e`/`fase33z_c` producer + stream-effect, `fase33x_d`/`fase33z_k_h`/`fase34_a` streaming-tool path enforcement-summary + metadata + unified handler). Adopter docs: `docs/ADOPTER_AXONSTORE.md` gained §16 "Streaming the agent pattern (v1.35.0)" (16.1 the canonical `retrieve → step Deliberate → persist` agent flow recipe behind `axonendpoint transport: sse`; 16.2 `backend: in_memory` zero-infra; 16.3 `${interpolation}` data threading; 16.4 the exactly-one-terminator wire contract; 16.5 honest failure) + TOC entry. New `docs/MIGRATION_v1.35.md` (v1.34.x → v1.35.0 — 6-row change table, the one intended behavior change explained, 5 migration scenarios A–E, the D5 "what does NOT change" list, an upgrade checklist). Doc/CI-only sub-fase — no production change. |
| **36.x.h** | Coordinated release axon-lang v1.35.0 cross-stack (crates.io + PyPI + GitHub Release + binaries) + axon-enterprise catch-up. | SPLIT | — | ✅ SHIPPED — axon-lang **v1.35.0** live cross-stack: `bump-my-version` minor 1.34.0→1.35.0 (6 files) + axon-frontend 0.16.0→0.17.0 (the `in_memory` catalog add is a frontend change) + axon-rs dep pin `=0.16.0`→`=0.17.0`; release commit `2b0d197`; tags `v1.35.0`/`rust-v1.35.0`/`axon-frontend-v0.17.0` pushed; cargo publish axon-frontend 0.17.0 then axon-lang 1.35.0; GitHub Release v1.35.0 published with content-first notes + 5-platform binaries (linux x86_64/aarch64, macos x86_64/aarch64, windows x86_64) attached by `rust_release.yml`; PyPI `publish.yml` fired via draft-toggle recovery (the dropped-`published`-event quirk) + completed. Pre-release sweep: a stale in-module `streaming_via_dispatcher` unit test (36.x.c missed it — still asserted the old double terminator) was found + fixed (commit `603cd34`); axon-rs lib 2029/0, axon-frontend 262+/0, 36.x suite 21/21, Fase 33/34 streaming regression 48/48. axon-enterprise **v1.26.0** catch-up live: branch `feature/36x-axon-lang-1.35.0-catchup` → PR #31 merged commit `de43a86` (bump 2 files + dep pin `>=1.34.0`→`>=1.35.0`); annotated tag `v1.26.0`; Enterprise Release workflow built + pushed the multi-arch Docker image to ECR; GitHub Release v1.26.0 published with vertical-inheritance notes. |
