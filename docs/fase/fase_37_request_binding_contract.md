...
| **37.i** | Coordinated release axon-lang v1.36.0 cross-stack (crates.io + PyPI + GitHub Release + binaries) + axon-frontend bump (0.17.0 → 0.18.0 — the totality check is a frontend change) + axon-enterprise catch-up (v1.26.0 → v1.27.0). | SPLIT | — | ✅ SHIPPED — axon-lang **v1.36.0** live cross-stack: `bump-my-version` minor 1.35.0→1.36.0 (6 files) + axon-frontend 0.17.0→0.18.0 (the D2 totality check is a frontend change) + axon-rs dep pin `=0.17.0`→`=0.18.0`; release commit `52f9213`; tags `v1.36.0`/`rust-v1.36.0`/`axon-frontend-v0.18.0` pushed; cargo publish axon-frontend 0.18.0 (after a `description`-too-long trim — commit `6cbb090`, the v0.18.0 prepend hit the crates.io 1000-char cap; ancient v0.2.0–v0.8.0 notes collapsed to one line) then axon-lang 1.36.0; GitHub Release v1.36.0 published with content-first notes + 5-platform binaries attached by `rust_release.yml`; PyPI `publish.yml` fired via draft-toggle recovery + completed. Pre-release sweep: axon-frontend 262+ + axon-rs lib 2035 + 45 Fase 37 tests + 62 streaming/store/dynamic-route regression — all green. axon-enterprise **v1.27.0** catch-up live: branch `feature/37-axon-lang-1.36.0-catchup` → PR #32 merged commit `6d33187` (bump 2 files + dep pin `>=1.35.0`→`>=1.36.0`); annotated tag `v1.27.0`; Enterprise Release workflow built + pushed the multi-arch Docker image to ECR; GitHub Release v1.27.0 published with vertical-inheritance notes. **Fase 37 CLOSED.** |
