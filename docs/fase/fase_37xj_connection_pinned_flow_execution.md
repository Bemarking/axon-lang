...
| **37.x.j.11** | **POST-CLOSE HOTFIX #2** — `query()` / `insert()` / `mutate()` / `purge()` introspect-error masking. Diagnosed 2026-05-21. **Root cause**: when `introspect_conn(&mut tx, table)` fails inside the cache-MISS path of these 4 backend methods, the existing code emits `tracing::warn!(target: "axon::store", ...)` (Fase 38.x.a D3) but falls through with `(None, &no_types)` to bare-table SQL. The bare-table SQL runs in the SAME poisoned tx → fails with cascade `25P02 in_failed_sql_transaction` (or `42703 column does not exist`). The `StoreError::Query.source` returned to the application contains ONLY the secondary cascade error — the primary introspect cause is masked from any caller that doesn't filter the `axon::store` tracing target. **Adopter symptom**: receives `"transaction is aborted, commands ignored until end of transaction block"` with no idea WHAT command originally failed. **Why the fall-through existed**: theoretical corner case of adopters whose introspect privileges differ from query privileges (rare in practice — same DB user). **Fix — Option A (rollback + propagate)**: when introspect_conn fails, ROLLBACK the tx (best-effort `tx.rollback().await.ok()`) + return the introspect error directly via `From<IntrospectError> for StoreError`. Adopters now see the actual root cause at the FIRST failure. The corner case (introspect-without-privileges + query-with-privileges) becomes a future opt-in flag if real adopter demand surfaces. Applied UNIFORMLY at the 4 cache-MISS sites in [axon-rs/src/store/postgres_backend.rs](../../axon-rs/src/store/postgres_backend.rs) — `query()` line ~1438, `insert()` line ~1617, `mutate()` line ~1706, `purge()` line ~1795. Ships as axon-lang **v1.40.2 PATCH** + axon-enterprise **v1.31.2** catch-up alongside Fase 38.x.f.9 (generic-aware body validation). | Diagnostic clarity (D-letter-less; pure observability improvement) | ✅ SHIPPED 2026-05-21 — 4 sites in [axon-rs/src/store/postgres_backend.rs](../../axon-rs/src/store/postgres_backend.rs) converted from `match &resolved { Ok(r) => ..., Err(e) => { warn; (None, &no_types) } }` fall-through to `match introspect_conn(...).await { Ok(r) => r, Err(introspect_err) => { tracing::warn! with d_letter=37.x.j.11 + let _ = tx.rollback().await; return Err(introspect_err); } }`. The 4 sites — `query()` line ~1438, `insert()` line ~1617, `mutate()` line ~1706, `purge()` line ~1795 — now propagate the **primary** introspect error to the application layer instead of returning the **secondary** cascade error from a poisoned tx. Adopters see the actual root cause at the FIRST failure via the StoreError variant directly (no longer dependent on filtering the `axon::store` tracing target). The fall-through-with-empty-types-map (theoretical corner case for adopters with introspect ≠ query privileges) is removed; future fase can add `unsafe_skip_introspect` opt-in flag if real adopter demand surfaces. Anchor + lib tests preserved: **2114/2114** axon-lang lib (subió de 2108 — 6 nuevos gap 3 tests; gap 1 covered by existing fase35e_tests + production smoke) + **12/12** Fase 37.x.j anchor + **12/12** Fase 38.x.f anchor + **5/5** Fase 38.x.e cardinality = all green; zero regressions cross-stack. |
