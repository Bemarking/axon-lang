...
status: ✅ CLOSED 2026-05-12 — Fase 33 cycle fully shipped end-to-end. axon-lang v1.24.0 cross-stack live (master 4f81170 + release a1fffc6 + tags v1.24.0 / rust-v1.24.0 / PyPI 1.24.0 / crates.io axon-lang 1.24.0 / GitHub Release v1.24.0 with content-first notes); axon-enterprise v1.14.0 catch-up live (PR #16 merged commit d40b952 + tag v1.14.0 via refspec mapping + GitHub Release with vertical-inheritance notes for HIPAA/legal/fintech/government). D1–D10 RATIFICADAS bloque; 9 sub-fases 33.a-i; ~570 LOC across the four-layer architecture (33.a-e) + ~110 LOC cancel-safety (33.f) + ~520 LOC D12 fuzz (33.g) + ~540 LOC adopter docs (33.h) + coordinated release (33.i). **182 new tests across the Fase 33 cycle**; ~2 000 deterministic fuzz iterations; zero regressions across 178+ Fase 30/31/32 SSE-family + 1454+ axon-rs lib + full Python suite. Trigger = adopter MIGRATION_TO_AXON.md trail 2026-05-12 post-v1.23.1 ("10 bumps en 5 días. Mismo resultado. […] es un piece de arquitectura que necesita su propio cycle"). Founder principle "SSE es una primitiva cognitiva, eso en axon lo es todo y debe funcionar perfecto" honored end-to-end.
...
| **33.h** | Adopter docs | ~270 (ADOPTER_STREAMING.md §"Real-time streaming (Fase 33, v1.24.0+)" appended — TL;DR table + four-layer plain-language description + D9 wire byte-compat statement + `stream_policies` field surface + closed-policy-catalog table + per-provider streaming notes + migration recipe + honest scope statement re: 33.x + vertical patterns + cancel-safety adopter-visible effects + bug-filing recipes) + ~270 (new `docs/MIGRATION_v1.24.md` — 5 scenarios A-E covering server-only upgrade, observability indexing of `stream_policies`, per-provider streaming activation, pre-33.c burst-timing regression check, external-scope binding via `CancellationFlag`/`CancelOnDrop`; D9 anchor + honest scope statement re: 33.x) | ✅ SHIPPED 2026-05-12 — Adopter-facing documentation locks the Fase 33 contract from the adopter's perspective. ADOPTER_STREAMING.md grew from 1 215 → 1 484 LOC (+269) with a new top-level §"Real-time streaming (Fase 33, v1.24.0+)" anchored from the TOC; MIGRATION_v1.24.md (271 LOC) is the canonical scenario-driven upgrade guide. Both documents lead with the **D9 wire byte-compat statement**: every adopter contract built against the Fase 30/31/32 wire shape continues to work bit-identically; the new `stream_policies` field on `axon.complete` is elided when empty so unknown-field-ignoring parsers see no observable change. The honest scope statement re: 33.x ("server_execute_full materializes all step outputs before chunking; producer/consumer split is structurally end-to-end but per-token wall-clock incrementality activates when Backend::stream() wires into per-step execution") is published verbatim so adopters know the difference between "architecturally in place" and "adopter-observable end-to-end". Cross-references: ADOPTER_STREAMING ↔ MIGRATION_v1.24 ↔ STREAM_EFFECTS ↔ cancel_token.rs ↔ stream_effect_dispatcher.rs ↔ fase33_fuzz.rs — all six surfaces hyperlinked from the new sections. |
| **33.i** | Release | release | ✅ SHIPPED 2026-05-12 — **axon-lang v1.24.0 cross-stack live**: bump-my-version minor 1.23.1 → 1.24.0 (6 files: pyproject.toml + axon/__init__.py + axon-rs/Cargo.toml + axon-rs/Cargo.lock + 2 test pins); axon-frontend stays at 0.11.1 (no AST changes in this cycle — honest version discipline per `feedback_versioning_discipline`); commit `a1fffc6` with content-first release notes; tags `v1.24.0` + `rust-v1.24.0` pushed to origin; cargo publish axon-lang 1.24.0 to crates.io (4m29s release build, dry-run validated, uploaded successfully); GitHub Release `v1.24.0` published with content-first notes (auto-created from tag push, edited via `gh release edit` per the established quirk-recovery); PyPI publish.yml fired via draft-toggle recovery (run 25713868397 conclusion=success in 3m39s). **axon-enterprise v1.14.0 catch-up live**: branched off enterprise/master at v1.13.1, bumped 2 files (pyproject.toml version+dep pin + axon_enterprise/__init__.py `__version__`), commit `c3f6eda` on `release/enterprise-v1.14.0`; PR #16 opened + merged commit `d40b952`; tag `v1.14.0` via refspec mapping `git push origin v1.14.0:refs/tags/v1.14.0`; GitHub Release `v1.14.0` published with **explicit vertical-inheritance notes** covering all 4 verticals — Healthcare (HIPAA Safe Harbor §164.514(b)(2) + 21 CFR Part 11 §11.10 PHI scrubber operates on every wire token), Legal (FRE 502 reasoning-trace streams token-by-token, replay binding for appellate review), Fintech (PCI DSS Req 10 hash-chained via `axon_csys_enterprise::audit_log_mmap`, BSA/OFAC signals preserved across stream), Government (FedRAMP AU-2 retention mapped to same mmap kernel, FOIA-retrievable live trace). Honest 33.x scope statement published verbatim: per-token shield streaming surface + streaming-audit replay token chain are documented 33.x follow-ups. axon-frontend transitively at 0.11.1 (unchanged). |
