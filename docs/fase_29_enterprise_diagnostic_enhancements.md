...
status: IN PROGRESS 2026-05-12 — D1–D10 RATIFICADAS bloque (founder verbatim "Te ratifico todos los D-Letters"); 29.a + 29.b + 29.c SHIPPED on `feature/29b-vertical-diagnostic-policy` branch (axon-enterprise commits `645324b` + `32e53a8`); 29.d–29.i pending; target axon-enterprise v1.15.0 (next minor after the Fase 33 catch-up at v1.14.0)
...
| 29.d Vertical-aware suggest dictionaries | ⏳ pending | ~350 | Python + JSON | Extends OSS `_TOP_LEVEL_KEYWORD_NAMES` + `_FLOW_BODY_KEYWORD_NAMES` with vertical-specific aliases (no upstream change — wrapper layer). Curated from public-domain medical/legal/fintech terminology glossaries (verifiable provenance per dictionary entry, ~50 terms per vertical first-cut per D3 resolution). Wired via the `DiagnosticPolicy.extra_keywords` hook already plumbed in 29.b. Multi-vertical safe: `hipaa` policy loads medical dict only, never legal/fintech. |
