...
status: ✅ CLOSED 2026-05-12 — D1–D12 RATIFICADAS bloque; ALL 12 sub-fases 32.a–32.l SHIPPED end-to-end cross-stack. axon-lang v1.23.0 SHIPPED 2026-05-11; v1.23.1 patch SHIPPED 2026-05-12 closing the Rust parser disjunct (a) gap surfaced by adopter `docs/MIGRATION_TO_AXON.md` audit. Both versions live cross-stack (crates.io axon-frontend 0.11.0/0.11.1 + axon-lang 1.23.0/1.23.1 + PyPI axon-lang 1.23.0/1.23.1 + GitHub Releases v1.23.0/v1.23.1); axon-enterprise v1.13.0 + v1.13.1 catch-ups live (PRs #13 + #15 merged; tags v1.13.0/v1.13.1 via refspec; GitHub Releases published). axon-enterprise PR #14 commit `9d7fe12` hardened the Enterprise Release Docker build's Wait step to do a two-phase CDN-consistency probe (HEAD + 3 consecutive range-GETs spaced 15s) so the v1.13.0 CDN-edge race condition cannot repeat
...
| 32.j Adopter documentation surface (D10) | ✅ SHIPPED 2026-05-11 | ~840 (new `docs/ADOPTER_REST.md` 18-section comprehensive guide) + ~270 (new `docs/MIGRATION_v1.23.md` 6-scenario migration recipe) + ~95 (`docs/ADOPTER_STREAMING.md` §"Dynamic routes (Fase 32, v1.23.0+)" extension) + ~180 (`docs/ADOPTER_DIAGNOSTICS.md` Pattern 7 with 8 sub-patterns) = **~1385 LOC of new + extended adopter docs** | Docs | **`docs/ADOPTER_REST.md`** (new, 837 LOC): 18-section comprehensive guide covering the whole Fase 32 surface for adopters: (1) v1.23.0 changes table, (2) first axonendpoint REST route, (3) every field of the declaration, (4) body schema validation with type-shape reference + dotted/bracket field_path notation, (5) output schema validation with OWASP-safe 500 envelope + audit log retrieval recipe, (6) per-endpoint transport matrix on dynamic routes, (7) Idempotency-Key Stripe-compatible contract with conflict diagnostic + cross-tenant isolation, (8) auth scope `requires:` with closed slug grammar + 403 deny envelope + OSS vs enterprise capability surface, (9) replay-token binding with `GET /v1/replay/<id>` retrieval + deterministic-vs-non-deterministic distinction + 30d retention, (10) multi-endpoint deployment + path collision diagnostics, (11) EventSource on arbitrary paths, (12) **5-vertical cookbook** with regression-tested canonical patterns (banking PCI DSS Req 10 LoanDecision + government FedRAMP AU-2 BenefitsEligibility + legal FRE 502 DiscoveryPrivilege + medicine HIPAA + 21 CFR Part 11 CDSEndpoint + generic), (13) D10 backwards-compat note (/v1/execute preserved), (14) production deployment checklist (7-step pre-flip audit), (15) troubleshooting (5 common-symptom recipes), (16) cross-stack contract reference, (17) four-pillar trace, (18) bug-filing pointers. **`docs/MIGRATION_v1.23.md`** (new, 269 LOC): 6-scenario migration recipe: Scenario A (Kivi case quick-fix) + Scenario B (body schema validation) + Scenario C (banking Idempotency-Key retries) + Scenario D (auth-scoped regulated endpoints) + Scenario E (replay-token binding for regulatory audit) + Scenario F (disable dynamic routes opt-out); compatibility matrix v1.20.x–v1.23.0 clients. **`docs/ADOPTER_STREAMING.md`** extension (+95 LOC): §"Dynamic routes (Fase 32, v1.23.0+)" before "Where to file bugs" — per-route negotiation matrix + keepalive on declared paths + X-Axon-Stream-Available diagnostic on dynamic routes + Replay-Status header semantics + SSE-bypasses-validation note. **`docs/ADOPTER_DIAGNOSTICS.md`** Pattern 7 (+180 LOC): 8 sub-patterns each anchored to a D-letter — 7.1 path collision (D2) + 7.2 body schema violation (D4) + 7.3 invalid method/path (D3) + 7.4 missing capability (D8) + 7.5 Idempotency-Key reuse (D7) + 7.6 unknown path (404) + 7.7 internal validation error (D5 OWASP-safe) + 7.8 replay trace not found (D9); each sub-pattern shows the verbatim error envelope + the fix. Vertical-specific enterprise docs (`docs/enterprise/REST_BANKING.md` + `REST_GOVERNMENT.md` + `REST_LEGAL.md` + `REST_MEDICINE.md`) ship as part of axon-enterprise v1.13.0+ catch-up release (32.k follow-on), not OSS. **Pillar trace**: PHILOSOPHY — adopters self-onboard with the source of truth as docs (not Slack DMs); the language honors its own declarations + the docs make that visible; COMPUTING — every D-letter referenced is traceable from docs back to source. **Cross-references**: every doc cross-links to siblings + to the plan vivo + to the regression test packs |
| 32.k Coordinated cross-stack release v1.23.0 | ✅ SHIPPED 2026-05-11 | release | All stacks | **axon-lang v1.23.0 LIVE end-to-end**: bump-my-version minor 1.22.0 → 1.23.0 (6 files: pyproject.toml + axon/__init__.py + axon-rs/Cargo.toml + 3 test asserts) + manual bump axon-frontend 0.10.0 → 0.11.0 + axon-rs dep pin =0.10.0 → =0.11.0; commit `f3b106a` + 3 tags `v1.23.0` + `rust-v1.23.0` + `axon-frontend-v0.11.0` pushed to origin; `cargo publish` axon-frontend 0.11.0 (description-too-long fix in `3358952` followup) + `cargo publish` axon-lang 1.23.0 both live on crates.io; GitHub Release v1.23.0 published with content-first notes (highlights table + D-letter recap + adopter docs links + test surface counts + founder principle); PyPI publish.yml workflow fired via draft-toggle recovery + completed (run 25703482019). **axon-enterprise v1.13.0 catch-up LIVE**: branch `release/enterprise-v1.13.0` off enterprise/master with 2-file bump (`pyproject.toml` 1.12.0 → 1.13.0 + dep pin `axon-lang>=1.22.0` → `>=1.23.0` + `axon_enterprise/__init__.py` 1.12.0 → 1.13.0); PR [#13](https://github.com/Bemarking/axon-enterprise/pull/13) merged commit `8dc3e07`; tag `v1.13.0` via refspec mapping `enterprise/v1.13.0:refs/tags/v1.13.0` pushed; GitHub Release v1.13.0 published with vertical-relevance notes (HIPAA + Legal + Fintech + Government inheriting Fase 32 surface). Same lean-bump shape as v1.9.0 + v1.11.0 + v1.12.0 — no enterprise code changes; substantive Fase 27.k.1 Python ctypes integration + vertical-specific enterprise docs (REST_BANKING + REST_GOVERNMENT + REST_LEGAL + REST_MEDICINE) tracked separately. Founder directive "Hacer que una aplicación AI sea determinista..." honored end-to-end at the wire layer |
| 32.l Patch — Rust parser disjunct (a) for `output: Stream<T>` in step body | ✅ SHIPPED 2026-05-12 | ~50 (Rust parser `parse_output_type_string` helper + step `output:` arm) + ~290 (axon-frontend test pack `tests/fase32_l_stream_output_in_step.rs` 10 tests) + ~225 (axon-rs E2E test pack `tests/fase32_l_kivi_disjunct_a_end_to_end.rs` 4 tests) + release (v1.23.1 + axon-frontend v0.11.1 + enterprise v1.13.1) | Rust + release | **Adopter trail closes the last cable**: post-v1.23.0 adopter `docs/MIGRATION_TO_AXON.md` audit 2026-05-12 reported that dynamic routes serving stream-effect flows declared via disjunct (a) — `step S { output: Stream<T> }` — STILL returned `application/json` instead of `text/event-stream`. **Root cause**: `Parser::parse_step` for `output:` consumed only a bare `Identifier` token, dropping the `<T>` generic. `Stream<Token>` became `"Stream"` in AST; downstream `flow_has_stream_output`'s `starts_with("Stream<") && ends_with('>')` predicate returned false → `produces_stream` false → `implicit_transport == "json"` → `classify_dynamic_route_wire` returned `Json` → dispatch served the JSON envelope. **Fix**: new `Parser::parse_output_type_string` (Python `_parse_output_type_string` Rust mirror — Python was fixed for the same gap 2026-05-09) accepts the FULL generic-aware shape `Identifier` / `Stream<T>` / `Optional?` / `Stream<T>?`. `parse_step`'s `Output` arm now calls this helper. D11 cross-stack parity restored. **Tests**: axon-frontend `tests/fase32_l_stream_output_in_step.rs` (**10/10 pass**) anchors parser shape + `produces_stream` + `implicit_transport` + `compute_implicit_transports` + explicit `transport: json` D3 override; axon-rs E2E `tests/fase32_l_kivi_disjunct_a_end_to_end.rs` (**4/4 pass**) deploys the EXACT adopter-canonical disjunct-(a) source and asserts the dynamic-route response Content-Type under strict mode (SSE) + legacy+Accept SSE (D4 fallback) + legacy without Accept (JSON D9 + X-Axon-Stream-Available diagnostic header fires — the silent absence pre-fix was the adopter's diagnostic wall) + explicit `transport: sse` belt-and-suspenders. **Releases**: bump-my-version patch 1.23.0 → 1.23.1; axon-frontend 0.11.0 → 0.11.1; commit `3e1dde3` + 3 tags pushed; cargo publish axon-frontend 0.11.1 + axon-lang 1.23.1 live on crates.io; GitHub Release v1.23.1 published with adopter trail + before/after table; PyPI publish.yml fired via draft-toggle (run 25706646660). **axon-enterprise v1.13.1 catch-up LIVE**: PR [#15](https://github.com/Bemarking/axon-enterprise/pull/15) merged commit `1eaca58`; tag `v1.13.1` via refspec; GitHub Release v1.13.1 published. **Enterprise Release CDN-consistency hardening**: PR [#14](https://github.com/Bemarking/axon-enterprise/pull/14) commit `9d7fe12` shipped the two-phase Wait step (HEAD + 3 consecutive range-GETs spaced 15s) so the v1.13.0 CDN-edge race condition CANNOT REPEAT; v1.13.1 was the first organic exercise of the hardened gate. Pillar trace: PHILOSOPHY — adopter MIGRATION_TO_AXON audit is the language's signal; LOGIC — declarative source IS the wire format; MATHEMATICS — `produces_stream` total over disjunct (a) restored; COMPUTING — D11 cross-stack parity restored, no behavior change for sources NOT using disjunct (a). **Combined regression at 32.l close**: 1447 axon-rs lib + 246 axon-frontend lib (+10 32.l unit) + 169 Fase 30/31/32 Rust integration (+4 32.l E2E = 173 with 32.l) + 6691 Python (5 skipped) verde |
