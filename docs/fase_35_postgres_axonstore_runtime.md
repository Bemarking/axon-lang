...
| **35.e** | Wire into the sync runner | ~450 | ⏳ pending | `runner.rs` — `execute_server_flow` builds the `StoreRegistry` + threads it into `execute_real`; the session-memory interception consults it. KV path byte-unchanged (D3). |
