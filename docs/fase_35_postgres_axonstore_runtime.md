...
| **35.d** | `StoreRegistry` + closed-catalog dispatch | ~450 | ⏳ pending | New `axon-rs/src/store/registry.rs` — built from `IRProgram.axonstore_specs`; resolves `store_name` → `IRAxonStore`; closed `backend` dispatch (`postgresql` → SQL backend, else → KV path); per-DSN pool cache. D2 + D3 — the single SQL-vs-KV chokepoint. |
