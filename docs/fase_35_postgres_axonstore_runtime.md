...
| **35.f** | Wire into the streaming dispatcher | ~450 | ⏳ pending | `flow_dispatcher/wire_integrations.rs` — `persist_to_store`/`retrieve_from_store`/`mutate_store`/`purge_from_store` consult the registry (threaded via `DispatchCtx`). Production hot path. D3 + D5. |
