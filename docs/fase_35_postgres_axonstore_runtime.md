...
| **35.b** | Filter compiler — `where`-expr → parameterized SQL | ~600 | ⏳ pending | New `axon-rs/src/store/filter.rs` — tokenizer + parser → closed-catalog `FilterCondition` AST → `build_pg_where(expr, offset) -> (clause, Vec<SqlValue>)` with `$N` placeholders. Pure, no I/O, exhaustively unit-tested. D4. |
