...
status: ✅ CLOSED 2026-05-11 — axon-lang v1.22.0 cross-stack live (PyPI + crates.io axon-lang 1.22.0 + crates.io axon-frontend 0.10.0 + GitHub Release v1.22.0 with content-first notes + 6 binary tarballs + SHA256s); axon-enterprise v1.12.0 catch-up live (PR #12 merged commit `42719ec`, tag pushed via refspec `enterprise/v1.12.0:refs/tags/v1.12.0`, GitHub Release published); D1–D10 RATIFICADAS en bloque; 9 sub-fases 31.a–31.i shipped end-to-end
...
|---|---|---|---|---|
| 31.a Engineering spec + D-letter ratification | ✅ SHIPPED 2026-05-11 | doc-only | — | This doc + memory entry `project_fase_31_plan.md` + MEMORY.md index update. Founder bloque approval 2026-05-11 ("aprobadas todas D-letters") locks D1–D10 verbatim. Founder principle reinforced: every D-letter traces to ≥ 1 of the four pillars (D10 meta) |
| 31.b Type-checker — implicit_transport(E) inference | ✅ SHIPPED 2026-05-11 | ~115 (Python `_implicit_transport` + `_compute_implicit_transports` + walker extensions) + ~190 (Rust mirror) + ~30 (AST + parser field additions) + ~360 (Python test pack) + ~210 (Rust drift gate) + ~95 (corpus 25 entries) | Python + Rust | **AST extension** — `AxonEndpointDefinition` gains two fields on BOTH stacks: `transport_explicit: bool` (parser sets true when source declares `transport:`) + `implicit_transport: str` (type-checker computes per D1). **Python type-checker** — `_implicit_transport(endpoint, flow, symbol_lookup) -> str` + `_compute_implicit_transports(program, symbol_lookup)` wired as Phase 5 of `TypeChecker.check()`. **Rust mirror** — `pub fn implicit_transport(...) -> String` + `pub fn compute_implicit_transports(&mut Program)` in `axon-frontend/src/type_checker.rs`, free-standing for axon-rs runtime + axon-lsp consumption. **`_flow_uses_streaming_tool` extended** to resolve BOTH `use_tool` AND `apply_ref` references (the Kivi-shape pattern 2026-05-11 that Fase 30.c missed — disjunct (b) closure now matches the source-text classifier's coverage). **Cross-stack drift gate** — 25-entry corpus at `tests/fixtures/fase31_implicit_transport/corpus.json` parametrized over both stacks. **Test packs**: Python `tests/test_fase31_implicit_transport.py` (**49/49 pass**: 4 corpus integrity + 25 drift parametrized + 5 disjunct (a/b) positives + 2 negative space + 3 explicit-wins + 3 transport_explicit flag + 3 edge cases + 2 multi-endpoint isolation + 1 idempotence + 1 string-literal-stream-pattern-doesnt-trigger) + Rust `axon-frontend/tests/fase31_drift_gate.rs` (**5/5 pass**: corpus shape + cross-stack inference + parser flag + idempotence + unresolvable-flow defaults). **Known Rust parser gaps marked**: 3 corpus entries flagged `rust_parser_known_gap: true` (`output: Stream<T>` in step body + `use Tool()` at flow body — both Fase 30.e known gaps awaiting Rust frontend completion sub-fase). Rust drift gate skips flagged entries + asserts ≥ 15 cross-stack entries (currently 22 of 25 active). **Regression cross-stack**: Python 5385 existing + 50 new Fase 31; Rust 239 axon-frontend lib + 5 fase31 drift; axon-rs 1385 lib — zero regressions. **Pillar trace**: MATHEMATICS (total + deterministic function) + LOGIC (disjunction of 3 disjuncts) + PHILOSOPHY (language is wire source of truth) + COMPUTING (idempotent, defensive against malformed AST) |
| 31.c Compile-time warning emission for implicit transport (rolling-out diagnostic) | ✅ SHIPPED 2026-05-11 | ~180 (Python `_describe_stream_origin` + `_build_w001_message` + `_emit_implicit_transport_warnings`) + ~165 (Rust mirror) + ~270 (Python test pack) + ~225 (Rust test pack) | Python + Rust | **Warning code namespace `axon-Wnnn` opened**: `axon-W001` is the first entry (errors keep their `axon-Ennn` from Fase 28 + Fase 30). Python `_emit_implicit_transport_warnings` walks every `AxonEndpointDefinition`, emits `axon-W001` when (1) `transport_explicit == False` AND (2) `implicit_transport == "sse"` AND (3) `execute_flow` resolves to a flow in scope. Wired as Phase 6 of `TypeChecker.check()` AFTER Phase 5 (inference). Rust mirror `compute_implicit_transport_warnings(&Program) -> Vec<TypeError>` shares the predicate + message shape byte-identically. **Message shape (D4 + plan vivo §6.2)**: `warning[axon-W001]: implicit \`transport: sse\` inferred from stream effects on axonendpoint 'X' (flow 'Y' produces a stream via <origin>). Declare \`transport: sse\` to silence this warning and lock in SSE behavior, or \`transport: json\` to opt out and keep the legacy JSON wire format. When \`strict_type_driven_transport: true\`, this endpoint emits SSE on /v1/execute by default.` Origin is disjunct-specific: `step '<n>' has output: Stream<T>` for (a) / `step '<n>' applies tool '<t>' with effects <stream:<policy>>` for (b) / `perform Stream.Yield(...) inside step '<n>'` for (c). **Rate-limiting** by construction (each axonendpoint visited once). **Suppression matrix verified**: explicit `transport: sse|json|ndjson` declared, no stream effects, orphan `execute_flow`. **Strict mode promotion** (Fase 28.h) happens at the rendering layer — no changes needed in the warning emitter. **Test packs**: Python `tests/test_fase31_implicit_transport_warning.py` (**21/21 pass**: 6 positive (Kivi shape + disjunct (a) + 4 backpressure policies) + 5 suppression (each explicit value + no-stream + orphan) + 3 rate-limit (single + multi-endpoint + mixed) + 4 message shape (prefix + remediation + strict flag + line/column) + 1 idempotence + 2 origin description). Rust `axon-frontend/tests/fase31_warning.rs` (**16/16 pass**: same structural coverage + 1 new test `message_shape_matches_python_canonical_substrings` that anchors the cross-stack D7 contract on 5 verbatim phrases). Zero regressions cross-stack |
| 31.d Runtime — type-driven default behind feature flag | ✅ SHIPPED 2026-05-11 | ~30 (ServerConfig field + thread through 9 test helper constructors) + ~25 (negotiation classifier extension) + ~390 (test pack `axon-rs/tests/fase31_strict_mode.rs`) | Rust | **ServerConfig extension**: `pub strict_type_driven_transport: bool` added to [`axon-rs/src/axon_server.rs`](../axon-rs/src/axon_server.rs#L182) with extensive doc-comment trace to D6+D9 + the 31.f opt-in surfaces (CLI, env, config file). All 9 ServerConfig construction sites (`main.rs` + `tests/{fase30_*.rs × 4, integration.rs, axon_server.rs::tests × 2}`) updated to set the field — default `false` per D6 v1.22.x semantics. **Classifier extension** ([axon_server.rs `execute_handler_with_negotiation`](../axon-rs/src/axon_server.rs#L18418)): reads `state.config.strict_type_driven_transport` once + extends the `promote_sse` decision to `has_force_decl \|\| client_wants_sse \|\| strict_mode` when `decision == PromoteToSse`. The `StayJson` branch is sticky in both modes — D3 explicit opt-out is sacred. **D8 backwards-compat verified**: with the flag off, the existing Fase 30 D4 + D5 + D9 negotiation matrix is preserved byte-identically (every Fase 30 test still passes verbatim). **Test pack `axon-rs/tests/fase31_strict_mode.rs`** (**17/17 pass**): 8 strict-mode cells (stream × declaration × Accept matrix), 5 legacy-mode cells (D4 + D5 + D9 preservation), 1 12-cell decision-matrix summary test, plus 3 philosophical tests (`d3_opt_out_is_sacred_across_modes`, `d8_backwards_compat_v1_20_x_client_unchanged_when_flag_off`, `d1_inference_fires_only_when_predicate_satisfies` — the latter guards against the strict flag becoming a hammer). Hermetic via stub backend. **Cross-stack regression**: 1385 axon-rs lib + 49 Fase 30 integration (sse_runtime + negotiation + keepalive + sse_fuzz) + 17 new Fase 31 strict-mode = zero regressions. Python untouched (this sub-fase is Rust-only — the inference + warning live cross-stack from 31.b/c, this just adds the runtime path that consumes them) |
| 31.e Runtime — diagnostic response header `X-Axon-Stream-Available` | ✅ SHIPPED 2026-05-11 | ~75 (Rust handler change + `flow_has_any_stream_evidence` helper) + ~415 (test pack `axon-rs/tests/fase31_diagnostic_header.rs`) | Rust | **Header emission logic** ([axon_server.rs:18482](../axon-rs/src/axon_server.rs#L18482)) — after the legacy JSON handler returns the response, the wrapper checks whether the flow has ANY stream evidence (dual-signal: AST `flow_produces_stream_runtime` + source-text patterns — matches the architecture of `classify_negotiation_via_source_text` but **ignores** the `transport:` declaration since the diagnostic must fire ALSO for D3 opt-out adopters who declared `transport: json` while their flow still produces a stream). When evidence is present, the wrapper injects `x-axon-stream-available: 1; reason=<flag_off\|declared_json>; flow=<name>; opt_in=transport:sse,Accept:text/event-stream` into the response headers. **Reason vocabulary (closed set per D5)**: `flag_off` = strict flag is off + no Accept SSE (legacy v1.20.x path); `declared_json` = adopter explicitly opted out via `transport: json` (D3 sacred). **Never emitted** on SSE responses (wire is already streaming), non-stream flows (nothing to surface), or orphan endpoints (separate error path). **Header is informational only** — body byte-identical to pre-31.e responses; clients that ignore the header see zero difference. **Header value cleared via `HeaderValue::from_str` validation** so a flow name with CR/LF or illegal bytes silently skips the header rather than crashing the request. **Test pack `axon-rs/tests/fase31_diagnostic_header.rs`** (**15/15 pass**): 1 Kivi-shape fires correctly + 3 D3 opt-out scenarios (legacy / strict / with Accept SSE) + 6 suppression (SSE responses × 3 paths + non-stream × 2 + orphan) + 1 header-shape structural check + 1 closed-reason-set guard + 1 multi-endpoint flow-name isolation + 1 header-doesnt-mutate-body + 1 reason-flag_off-only-when-strict-off determinism. **Regression cross-stack**: 8 + 12 + 25 + 4 + 17 + 15 = 81 Fase 30 + Fase 31.d/e integration tests verdes. 1385 axon-rs lib verdes. Python untouched (runtime sub-fase, Rust-only) |
| 31.f CLI flag + server config surface | ✅ SHIPPED 2026-05-11 | ~50 (Rust CLI flag + `parse_truthy_env` helper) + ~45 (Python CLI flag + `_parse_truthy_env` mirror + `AxonServerConfig` field) + ~180 (Rust `tests/fase31_flag_surface.rs`) + ~180 (Python `tests/test_fase31_flag_surface.py`) | Python + Rust | **Two converging opt-in surfaces shipped** (CLI flag + env var); the config-file path was scoped out for 31.f and deferred to a future fase (the existing JSON `ConfigSnapshot` persistence requires touching PATCH /v1/config + serde + load/save which would balloon 31.f beyond its discrete scope — production deployments universally use either CLI args via k8s/docker OR env vars via 12-factor; the two-surface delivery covers both ergonomic paths). **Cross-stack contract (D7) anchored on the env var name**: `AXON_STRICT_TYPE_DRIVEN_TRANSPORT` accepted byte-identically by Python `axon serve` and Rust `axon-rs`. Truthy alphabet is intentionally constrained — `{"1", "true", "yes", "on"}` only, case-insensitive, whitespace-trimmed. Variants like `"y"`, `"t"`, `"enabled"`, `"active"` are NOT accepted (silent interpretation drift refused). **Rust** ([axon-rs/src/main.rs Commands::Serve](../axon-rs/src/main.rs) + [axon_server.rs::parse_truthy_env](../axon-rs/src/axon_server.rs)): clap flag `--strict-type-driven-transport` on `axon serve`; env var fallback via `parse_truthy_env`; CLI wins via `cli_flag \|\| env_var` short-circuit. **Python** ([axon/cli/__init__.py](../axon/cli/__init__.py) + [axon/cli/serve_cmd.py](../axon/cli/serve_cmd.py) + [axon/server/config.py](../axon/server/config.py)): argparse `--strict-type-driven-transport` (action store_true); `_parse_truthy_env` helper mirroring Rust shape byte-identically (same `_TRUTHY_VALUES` frozenset, same lowercase+strip); `AxonServerConfig.strict_type_driven_transport: bool = False` field; startup banner surfaces the resolved state. Python runtime currently does NOT yet act on the field (Fase 30/31 SSE handlers are Rust-only); field stored for cross-stack symmetry — when Python ports the negotiation path in a future fase, the wiring is already in place. **Tests cross-stack**: Rust `axon-rs/tests/fase31_flag_surface.rs` (**12/12 pass**: truthy accept matrix + falsy reject matrix + unset + whitespace + default + explicit + 4-cell precedence + canonical name anchor + alphabet-closed guard) + Python `tests/test_fase31_flag_surface.py` (**44/44 pass**: parametrized truthy/falsy + alphabet-closed + cross-stack env-name anchor + AxonServerConfig field default/explicit + 4-cell precedence + argparse integration including help-text mentions-env-var). **Regression**: 1385 axon-rs lib + 81 Rust integration + 5455 Python verdes (zero regressions). axon-frontend untouched (this sub-fase is CLI-only) |
| 31.g CI matrix + cross-stack drift gate + 100-iter behavior fuzz | ✅ SHIPPED 2026-05-11 | ~190 (YAML workflow) + ~225 (Python fuzz pack) + ~220 (Rust fuzz pack) | YAML + Python + Rust | New `.github/workflows/fase_31_type_driven_transport.yml` with **4 parallel lanes**: (1) **python-type-checker** (3.12 × 3.13 matrix; runs 31.b inference + 31.c warning + 31.f flag-surface + 31.g D12 fuzz packs); (2) **rust-frontend-mirror** (ubuntu × macos × windows matrix; runs 31.b drift gate + 31.c warning mirror + 31.g D12 fuzz); (3) **rust-runtime** (ubuntu; runs 31.d strict-mode + 31.e diagnostic header + 31.f flag surface + Fase 30 regression as explicit D8 backwards-compat anchor); (4) **cross-stack-drift** (explicit D7 parity: Python `TestPythonAgainstCorpus` + Rust `fase31_drift_gate` gate the same JSON fixture). **D12 budget extended to Fase 31** — 100-bucket × 10-iter deterministic fuzz on the inference pass. **Python fuzz `tests/test_fase31_implicit_fuzz.py` (225/225 pass)**: 3 classes × seeds — `TestImplicitTransportFuzzNeverPanics` (100 × 10 iters; `_compute_implicit_transports` never raises on lex+parse-clean inputs), `TestImplicitTransportFuzzClosedOutputSet` (100 × 10; `implicit_transport ∈ {"sse", "json"}` always — D2 closed set), `TestImplicitTransportFuzzIdempotence` (25 × 10; `f(f(x)) == f(x)` for the pass — mathematical idempotence). **Rust mirror `axon-frontend/tests/fase31_implicit_fuzz.rs` (3/3 pass)**: `inference_pass_never_panics_under_byte_fuzz` (100 × 10 iters; uses `std::panic::catch_unwind` for explicit panic surfacing; tests both `compute_implicit_transports` and `compute_implicit_transport_warnings`; sanity gate ≥ 100 iters reach the inference pass), `implicit_transport_always_in_closed_set_under_fuzz` (100 × 10; verifies closed-set output), `idempotence_holds_under_fuzz` (25 × 10). Same `Xorshift` PRNG + same seed numbering as Fase 28 + Fase 30 packs so cross-stack regressions reproduce on the same bucket. **Combined Fase 31-specific test count**: Python 49 + 21 + 44 + 225 = 339 + Rust 5 drift + 16 warning + 3 fuzz (frontend) + 17 strict + 15 header + 12 flag (runtime) = 68 = **407 Fase 31-specific tests verdes**. Full repo regression unchanged. **Pillar trace**: MATHEMATICS (inference function total + deterministic + idempotent) + LOGIC (output set closed) + COMPUTING (defensive cross-stack; drift impossible to ship; D8 D9 D7 anchors locked in CI) |
| 31.h Adopter documentation surface | ✅ SHIPPED 2026-05-11 | ~290 LOC new `MIGRATION_v1.22.md` + ~135 LOC extension in `ADOPTER_STREAMING.md` (new §Type-driven default transport section) + ~70 LOC extension in `ADOPTER_DIAGNOSTICS.md` (new Pattern 6) + ~5 LOC Fase 30 plan vivo back-link | Docs | **New `docs/MIGRATION_v1.22.md`** — top-level adopter migration guide for v1.21.x → v1.22.0. Four-scenario recipe (A: Kivi-shape quick-fix; B: opt-in flag for default-everywhere; C: D3 opt-out intentional JSON wrapping; D: staged rollout dev→staging→prod). Surfaces the `axon-W001` warning + `X-Axon-Stream-Available` header verbatim. Documents v2.0.0 D9 flag-flip with full migration window (v1.22.x + v1.23.x + …). Closing **What can't go wrong** section enumerates the 4 sacred invariants (D8 backwards-compat when flag off + D3 opt-out always wins + D7 cross-stack consistency + D10 four-pillar trace requirement) so adopters internalize the safety contract. 5-step troubleshooting checklist points back to the runtime header reasons + startup banner. **`ADOPTER_STREAMING.md` extension** — new "§ Type-driven default transport (Fase 31, v1.22.0+)" section inserted between Content-negotiation + Type-checker enforcement; contains the inference rule formalism (D1), the `axon-W001` warning shape + suppression rules + strict-mode promotion, the `X-Axon-Stream-Available` header with closed reason set, the two opt-in surfaces (CLI + env), the 16-cell decision matrix highlighting the bottom-right new Fase 31 row, and the D7 cross-stack contract paragraph. TOC updated (15 → 17 entries — adds Type-driven section + nudges back-half numbering). **`ADOPTER_DIAGNOSTICS.md` extension** — new Pattern 6 (NEW in v1.22.0) immediately after Pattern 5, walking adopters through the `axon-W001` example, the predicate firing conditions, the 3 suppression rules, and the 3 remediation paths (lock SSE / opt out / flip flag). Cross-links to `MIGRATION_v1.22.md` for the migration scenarios. **Fase 30 plan vivo back-link** — adds a "Successor plan vivo (Fase 31, v1.22.0+)" paragraph in the sibling-docs block referencing the philosophical extension. Adopters discovering Fase 30 docs now have a forward path to Fase 31 without spelunking the directory. **Philosophy + Computing**: the documentation surface treats adopter cognitive load as a first-class concern; migration is a 60-second source-edit OR a 1-line config flip; no adopter should EVER reach the 7-version-iteration wall the Kivi case revealed |
| 31.i Coordinated cross-stack release v1.22.0 | ✅ SHIPPED 2026-05-11 | release commit `d77cdb1` + 3 annotated tags + 2 crates.io publishes + GitHub Release with 130-line content-first notes + axon-enterprise PR #12 merged + enterprise tag refspec push + enterprise GitHub Release | All stacks | **axon-lang v1.22.0 live cross-stack**: PyPI [`axon-lang==1.22.0`](https://pypi.org/project/axon-lang/1.22.0/) + crates.io [`axon-lang 1.22.0`](https://crates.io/crates/axon-lang/1.22.0) + crates.io [`axon-frontend 0.10.0`](https://crates.io/crates/axon-frontend/0.10.0) + GitHub Release [v1.22.0](https://github.com/Bemarking/axon-lang/releases/tag/v1.22.0). bump-my-version minor handled the 5 axon-lang files atomically; manual bumps for `axon-frontend/Cargo.toml` (0.9.0 → 0.10.0) + axon-rs dep pin (`axon-frontend = { ..., version = "=0.10.0" }`); `cargo update --workspace` refreshed both lockfiles. 3 annotated tags created + pushed to origin: `v1.22.0` (Python release identifier triggering publish.yml) + `rust-v1.22.0` (axon-lang Rust crate alias triggering rust_release.yml multi-arch binary builds — 6 binary tarballs + SHA256s attached: linux-x86_64 + linux-aarch64 + macos-x86_64 + macos-aarch64 + windows-x86_64 + axon-frontend-wasm32) + `axon-frontend-v0.10.0` (informational for the frontend crate). Cargo publishes ran in dependency order: axon-frontend 0.10.0 first, then axon-lang 1.22.0. GitHub Release notes rewritten content-first per versioning discipline — 130-line markdown describing the inference rule formalism + W001 warning + diagnostic header + flag-gated rollout + cross-stack contract + D1-D10 trace + stack matrix + migration recipe + v2.0.0 advance notice + founder principle. Known PyPI publish.yml `release: published` trigger quirk surfaced (memory `project_axon_lang_publish_trigger_quirk.md`) — resolved via documented draft-toggle recovery (publish.yml then fired correctly in 5m05s and PyPI propagated `axon-lang 1.22.0` as latest). **axon-enterprise v1.12.0 catch-up live**: release branch `release/enterprise-v1.12.0` pushed; PR #12 opened with full body cross-referencing Fase 30 + Fase 31 cascade; CI matrix ran with 14/15 hard-green (TSan soft per documented rustc nightly quirk, same as v1.11.0); merged via `gh pr merge 12 --merge --admin` to commit `42719ec`; local annotated tag `enterprise/v1.12.0` created on merge commit; pushed via refspec mapping `enterprise/v1.12.0:refs/tags/v1.12.0` per `reference_enterprise_release_workflow.md`; GitHub Release [v1.12.0](https://github.com/Bemarking/axon-enterprise/releases/tag/v1.12.0) created manually with content-first notes (enterprise releases are manual — no auto workflow). Lean catch-up shape: 2 files (`pyproject.toml` + `axon_enterprise/__init__.py`) + dep pin `axon-lang>=1.20.0` → `>=1.22.0`. Zero enterprise-only code changes; the entire Fase 30 + Fase 31 surface inherits transitively. Regulated-vertical tenants (HIPAA / legal / fintech ensembles in `axon_enterprise.shield`) gain the streaming wire format automatically when the strict flag is on — no per-axonendpoint declaration churn. Founder principle "sabor consentido" honored: enterprise is alineado con axon-lang capabilities desde día 1 del v1.22.0 release. **Fase 31 closes**: 9 sub-fases acumuladas, all D-letters D1-D10 ratified, 407 Fase 31-specific tests verdes cross-stack, dedicated 4-lane CI workflow operational, comprehensive 220-LOC MIGRATION_v1.22.md + extensions in ADOPTER_STREAMING + Pattern 6 in ADOPTER_DIAGNOSTICS. The Kivi 7-iteration wall is impossible to reach again post-v1.22.0 |
