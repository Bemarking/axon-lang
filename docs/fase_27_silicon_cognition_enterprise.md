...
status: DRAFTED 2026-05-09 — investigación completada; sub-fases 27.a–27.k pendientes; target axon-enterprise v1.10.0
...
## ▶ Status snapshot (2026-05-09 — DRAFTED)

Investigación profunda terminada (ver §1). Plan operacional: §3 en adelante. Espera ratificación founder sobre decisiones D1–D14 antes de arrancar 27.b.

| Sub-phase | Status | LOC target | Stack | Module(s) / Notes |
|---|---|---|---|---|
| 27.a Engineering spec | ⏳ pending | doc-only | — | This doc + memoria `project_fase_27_plan.md` + D1–D14 ratificadas |
| 27.b axon-csys-enterprise crate scaffold + cc-rs build infra + license posture (D2 BSL with 4-year delay → MIT) | ⏳ pending | ~500 | Rust + C + build.rs | crate `axon-csys-enterprise` 0.1.0 (`Cargo.toml`, `build.rs`, `src/lib.rs`, `src/probe.rs`, `c-src/probe/probe.c`); BSL license file; cargo features `fips-boringssl` + `fips-openssl` mutually exclusive (default = no-fips passthrough that re-exports OSS axon-csys); ~10 tests |
| 27.c FIPS-validated crypto link (BoringSSL or OpenSSL-FIPS, per cargo feature) | ⏳ pending | ~800 (Rust shim + glue) | Rust + extern C | `axon-csys-enterprise/src/crypto.rs` re-exports OSS API surface (Sha256, HmacSha256, ContinuityWire) but routes the underlying primitives to the linked FIPS-validated lib; per-NIST-CAVS-vector drift gate vs OSS pure-C; the wire format stays byte-identical so flow output is interchangeable; ~30 tests |
| 27.d Audit log mmap append-only kernel | ⏳ pending | ~1500 (C + Rust shim + tests) | C + Rust shim | New `c-src/audit/log.{h,c}` + Rust shim `src/audit_log.rs` (`AuditLogWriter`, `AuditLogReader`, `AuditLogVerifier`); FlatBuffers wire format (D4 ratified — binary 10-20× smaller than JSON, mmap zero-copy reads); per-tenant Merkle hash chain with HMAC-SHA256 per-block + tenant-keyed seal (per-block detached signature via Ed25519 reserved for D5 public-anchor opt-in); time + size based rotation with ZIP archive of rotated segments; ~40 tests |
| 27.e Vertical BPE tokenizer templates (medical_base + legal_base + fintech_base) | ⏳ pending | ~600 (Python tooling + 3 .bin merges + Rust shim) | Python + C + Rust | New `axon-csys-enterprise/c-src/tokens/merges_<vertical>_base.bin` files baked via the OSS 25.g `#embed` infrastructure; Python training tool `tools/train_vertical_merges.py` consuming open corpora (PubMed Central / public-domain SEC filings / public US case law per D6 ratification); Rust shim registers `medical_base`, `legal_base`, `fintech_base` encoders alongside cl100k/o200k via `Tokenizer::from_blob`; existing `axon_enterprise.shield.ensemble_configs` factories detect + use the vertical encoders automatically (per D9 backward-compat); ~25 tests |
| 27.f Tamper-evident evidence packager (byte-deterministic ZIP + Merkle + Ed25519 seal) | ⏳ pending | ~1200 (C + Rust + Python wrapper) | C + Rust shim | Port + extension of OSS Tier 2B candidate (`axon-rs/src/esk/audit_engine/evidence_packager.rs`) to a fully byte-deterministic C23 ZIP encoder (STORE only, no DEFLATE) + per-file SHA-256 manifest + global Merkle root + per-tenant Ed25519 detached sig; Python wrapper in `axon_enterprise.audit.evidence_packager` that the supervisor + ESK already call; ~30 tests including byte-identical regen across runs |
| 27.g (optional, scope-cut candidate) PHI scrubber kernel in C23 | ⏳ pending | ~700 | C + Rust shim | Port of `axon_enterprise.shield.healthcare.hipaa_patterns` regex matcher to a SIMD-accelerated C kernel (Boyer-Moore-Horspool tabular search + AVX2/NEON for parallel match); useful when adopters stream 10k+ events/sec through the Shield. Scope-cut candidate if calendar tight; existing Python regex is fast enough for current adopter scale |
