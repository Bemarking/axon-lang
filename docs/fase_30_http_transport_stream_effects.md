...
status: ✅ CLOSED 2026-05-10 — axon-lang v1.21.0 cross-stack live (PyPI + crates.io axon-lang 1.21.0 + crates.io axon-frontend 0.9.0 + GitHub Release v1.21.0); D1–D8 RATIFIED en bloque; 9 sub-fases 30.a–30.i shipped end-to-end
...
| 30.b Parser `transport` + `keepalive` fields (Python + Rust + drift gate) | ✅ SHIPPED 2026-05-10 | ~80 (Py parser + AST) + ~140 (Rust parser + AST) + ~80 (corpus fixture) + ~320 (Py tests) + ~210 (Rust tests) | Python + Rust | Python: `AxonEndpointDefinition.transport: str = "json"` + `keepalive: str = ""` added to `axon/compiler/ast_nodes.py`; `_AXONENDPOINT_TRANSPORT_VALUES` + `_AXONENDPOINT_KEEPALIVE_VALUES` frozensets at module scope in `axon/compiler/parser.py`; `_parse_axonendpoint` extended with `transport` + `keepalive` cases that validate against the closed enums and emit smart-suggest hints (Fase 28.e) on rejection. Rust mirror: same field set in `axon-frontend/src/ast.rs`; `AXONENDPOINT_TRANSPORT_VALUES` + `AXONENDPOINT_KEEPALIVE_VALUES` `pub const` arrays in `axon-frontend/src/parser.rs`; mirror parser logic with same error-message shape. Drift-gate fixture `tests/fixtures/fase30_transport/corpus.json` (15 entries × 3-4 expected fields). Test packs: Python `tests/test_fase30_transport_parser.py` (37/37 pass: 4 integrity + 15 corpus parametrized + 3 defaults + 5 transport enum + 6 keepalive enum + 2 order-independence + 1 unknown-field skip + 1 unknown-field skip); Rust integration `axon-frontend/tests/fase30_drift_gate.rs` (8/8 pass: corpus shape + positive + negative + defaults + transport enum + keepalive enum + smart-suggest + invalid-keepalive). axon-frontend full regression: 251 (239 lib + 4 fase28 drift + 8 fase30 drift). axon-rs consumer crate builds clean (no struct-literal destructure sites). D1, D2, D6, D7 enforced |
| 30.c Type-checker enforcement (`transport: sse\|ndjson` requires Stream-producing flow) | ✅ SHIPPED 2026-05-10 | ~190 (type_checker.py predicate + integration) + ~370 (test pack) | Python | `axon/compiler/type_checker.py` ships the formal predicate as 4 pure helper functions: `_has_stream_output(step)` (disjunct a — type-level), `_has_stream_effect(tool)` (disjunct b — effect-level), `_is_stream_yield_perform(expr)` + `_walk_perform_expressions(node)` (disjunct c — operational), plus `_flow_produces_stream(flow, symbol_lookup)` master predicate as the disjunction. Walk discipline is O(N) over flow AST + one symbol-table lookup per use_tool. `_STREAMING_TRANSPORTS = {sse, ndjson}` frozenset; D2's ndjson is included because it's semantically streaming. `_check_axonendpoint` extended: when transport ∈ streaming AND execute_flow resolves to a FlowDefinition AND predicate fails → emit `AxonTypeError` with the 4-option remediation hint (Stream<T> output / stream effect / Stream.Yield / drop transport). Double-flag avoidance: skips when execute_flow already failed lookup. Test pack `tests/test_fase30_transport_typecheck.py` (31 pass + 1 skipped): TestDisjunctATypeLevel (4) covering Stream<T> shapes incl. optional-stream-does-not-count edge case + multi-step + inner-type-irrelevant; TestDisjunctBEffectLevel (6) covering all 4 ratified Fase 11.a policies + non-stream-effect-rejects + mixed-effect-row-accepts; TestDisjunctCOperational (3) covering simple yield + Stream.Done-does-not-count (tight match on Yield only) + perform-other-effect-rejects; TestNegativeSpace (3) covering empty body + non-stream output + non-stream tool; TestBackwardsCompat (3) covering absent transport + explicit json + streaming-flow-without-transport (no-check); TestStreamingTransports (3) covering D2 sse + ndjson both trigger + ndjson-with-disjunct-accepts; TestErrorMessageShape (4) pinning the §5.2 4-option hint; TestPredicateLogicProperties (3) pinning disjunction OR semantics + contrapositive + per-axonendpoint locality; TestEdgeCases (3) covering undefined-execute-flow no-double-flag + nested-yield (parser-limit-skipped) + yield-without-args. Rust frontend port deferred to 30.c.2 per Fase 18 decision (Python is canonical type-checker) |
| 30.d Runtime SSE single-shot path en `/v1/execute/sse` | ✅ SHIPPED 2026-05-10 | ~280 (handler + 4 SSE event builders) + ~340 (test pack) | Rust | New `POST /v1/execute/sse` route in `axon-rs/src/axon_server.rs` returning `axum::response::sse::Sse<Stream>`. Decision NOT to overload `/v1/execute` (preserves D9 backwards-compat strictly); explicit single-shot SSE route distinct from the two-stage `/v1/execute/stream` (D8 preserved). 4 event builders (`build_token_event`, `build_complete_event`, `build_error_event`, `build_retry_hint_event`) shipping the W3C SSE wire format per plan vivo §4: initial `retry: 5000` directive → per-step `event: axon.token\nid: <monotonic>\ndata: {...}\n\n` → final `event: axon.complete\nid: <last>\ndata: {final envelope}\n\n`. Mid-stream error → `event: axon.error\nid: <n>\ndata: {trace_id, error, recoverable:false}\n\n` + close. Reuses existing `StreamEmitter` algebraic-effect handler verbatim (same ~3-word chunking heuristic as the two-stage path; produces drift-free output between transports). Per-trace monotonic event-id counter (1-indexed for token/complete; retry directive carries no id per W3C convention). Tests `axon-rs/tests/fase30_sse_runtime.rs` (8/8 pass): route_exists_and_returns_text_event_stream + not_deployed_flow_emits_error_event_then_closes + deployed_flow_emits_token_events_and_complete + event_ids_are_monotonic (strict `>` between successive ids) + sse_wire_framing_w3c_compliant (ends with `\n\n` separator, no bare CR) + error_payload_includes_recoverable_flag + retry_directive_appears_first + empty_body_returns_some_response. Hermetic — uses the `stub` backend (zero network). Cancel-safety contract honored at axum + hyper layer: client disconnect drops the `Sse<Stream>` response; the executor today is synchronous so no background task to abort (future Fase candidate when executor becomes incremental, wire format does NOT change). axon-rs regression: **1385 lib tests + 8 new fase30_sse_runtime tests** = green across all 15 test lanes |
| 30.e Content-negotiation fallback (D4 safety net) | ✅ SHIPPED 2026-05-10 | ~280 (wrapper + classifier + 2 source-text helpers) + ~300 (test pack) | Rust | New `execute_handler_with_negotiation` wrapper on `POST /v1/execute` (legacy `execute_handler` preserved verbatim, called via fall-through). Inspects request `Accept` header + parses deployed source via `axon-frontend`. Negotiation decision combines two parallel signal paths to bridge known Rust-frontend parser gaps (`output: Stream<T>` and `use <tool>` inside step bodies don't yet surface in Rust AST; those gaps belong to a Rust frontend completion sub-fase): **(1) AST walk** of `classify_negotiation_for_flow` — canonical path for axonendpoint `transport:` field which the Rust parser handles correctly per 30.b drift gate; **(2) source-text predicate** `classify_negotiation_via_source_text` — defensive complement catching `output: Stream<`, `stream:drop_oldest`/`degrade_quality`/`pause_upstream`/`fail` substrings, `Stream.Yield(` operations. Decision logic: D5 explicit `transport: json` from either path is sticky (never override an explicit opt-out); otherwise PromoteToSse from either path wins. D4 promotion gated by `client_wants_sse` (Accept header) UNLESS D5 force-decl applies (`transport: sse|ndjson` always wins regardless of Accept). Wrapper returns `axum::response::Response` (heterogeneous: delegates to either legacy JSON handler via `.into_response()` OR `execute_sse_handler` from 30.d). Helper `source_text_axonendpoint_has_transport` does string-aware brace-counting to find the axonendpoint body even with embedded `path: "..."` strings. Test pack `axon-rs/tests/fase30_negotiation.rs` (**12/12 pass**): full decision matrix coverage — D5 force-sse with Accept JSON + without Accept header + ndjson; D5 explicit json suppresses even with Accept SSE; non-streaming flow returns JSON; D4 promotion via tool effect with all 4 ratified policies parametrized; D4 requires Accept (without → JSON); v1.20.0 backwards-compat verbatim; non-deployed-flow falls through cleanly; promoted SSE response carries retry directive + axon.complete event (wire-format integrity at the negotiation boundary). Hermetic via stub backend. Deployment-success assertion strengthened (`deploy_handler` returns HTTP 200 with `success: false` on parse failure — test helper now reads body, asserts `success: true`) |
| 30.f Keepalive comment emission (D6) | ✅ SHIPPED 2026-05-10 | ~210 (handler refactor + 4 pure helpers) + ~310 (test pack) | Rust | `axon-rs/src/axon_server.rs` ships the D6 keepalive surface via 4 pure helpers + a channel-fed-streaming refactor of `execute_sse_handler`. Helpers: `parse_keepalive_duration(s) -> Duration` maps the closed enum `{5s,15s,30s,60s}` (with D6 default 15s for empty/unknown — defensive against stale deployments); `lookup_keepalive_from_program(program, flow)` walks the parsed AST for the matching axonendpoint and returns its declared `keepalive` verbatim (`None` if no axonendpoint matches, `Some("")` if matched-but-omitted); `source_text_axonendpoint_keepalive(source, flow)` is the dual-signal fallback (Rust frontend parser gaps — same architecture as 30.e) with **string-aware brace counting** (path-literal `{`/`}` ignored) + **word-boundary anchoring** (`"5s"` substring of `"15s"` rejected); `resolve_keepalive_for_flow(source, flow)` is the single canonical entry point composing AST → source-text → D6 default. **Handler architecture refactor**: previous synchronous execute-then-emit pattern blocked the response body until execution completed → axum's `KeepAlive` had no inactivity window to fire into. 30.f spawns `server_execute_full` onto the blocking pool (`tokio::task::spawn_blocking`) and threads events through `futures::channel::mpsc::channel(16)`; the handler returns the Sse response IMMEDIATELY with the `retry:` directive pre-queued. Wire emission: `axum::response::sse::KeepAlive::new().interval(parsed_duration).text("keepalive")` → `: keepalive\n\n` per W3C SSE comment-line spec. Both deployed + not-deployed paths return through the unified channel (return type stays a single `impl Stream<Item = Result<Event, Infallible>>`). Cancel-safety: client disconnect drops Sse → drops `rx` → spawn task's `tx.send().await` returns SendError → swallowed via `.ok()` and the task exits cleanly. Test pack `axon-rs/tests/fase30_keepalive.rs` (**25/25 pass**): parse helper (4 — each enum value + empty + unknown + whitespace-trim); AST lookup (4 — explicit 5s + all-enum-values + omitted-field-returns-empty + non-matching-flow-returns-none); source-text helper (7 — space/no-space-after-colon + word-boundary-vs-15s + no-match + no-field + string-aware-braces + multi-axonendpoint-walk); resolve integration (4 — default-no-axonendpoint + default-no-field + each-enum-via-AST + multi-flow-isolation); 5 end-to-end through `/v1/execute/sse` (default-keepalive-wire-still-well-formed + declared-5s-still-well-formed + declared-60s-still-well-formed + not-deployed-path-still-works + fast-flow-does-not-emit-keepalive-comment). Hermetic via stub backend; tests do NOT assert emission timing (smallest enum is 5s, too slow for fast unit tests; axum's KeepAlive is mature/trusted production-grade impl, validated by 30.g 100-iter conformance fuzz). axon-rs full regression: **1385 lib tests + 8 sse-runtime + 12 negotiation + 25 keepalive** = 1430 green, zero regressions |
| 30.g CI matrix + cross-stack drift gate + SSE conformance fuzz | ✅ SHIPPED 2026-05-10 | ~180 (YAML workflow) + ~270 (Py fuzz pack) + ~260 (axon-frontend fuzz pack) + ~330 (axon-rs SSE fuzz pack) | YAML + Python + Rust | New `.github/workflows/fase_30_http_transport.yml` with **4 parallel lanes**: (1) **python-parser-and-typecheck** (3.12 × 3.13 matrix; runs 30.b parser pack + 30.c type-checker pack + 30.g fuzz pack); (2) **rust-frontend** (ubuntu × macos × windows matrix; runs 30.b cross-stack drift gate + 30.g axon-frontend parser fuzz); (3) **rust-runtime-sse** (ubuntu; runs 30.d single-shot SSE + 30.e content-negotiation + 30.f keepalive + 30.g SSE wire-format conformance fuzz); (4) **cross-stack-drift** (explicit D7 parity: corpus.json integrity + Python `TestPythonAgainstCorpus` + Rust `fase30_drift_gate` both gate the same JSON fixture). D12 budget extended to Fase 30 — 100-bucket × 10-iteration deterministic seed on every parser surface that touches `transport`/`keepalive`. **Test packs added**: Python `tests/test_fase30_transport_fuzz.py` (**200/200 pass**, 2 classes × 100 parametrized seeds — `TestTransportKeepaliveRobustnessFuzz` mixes valid-enum samples at 5% probability with adversarial identifier tokens, asserts closed-enum × closed-enum input round-trips fields verbatim and any other input either parses cleanly or raises `AxonParseError` — never an uncaught generic exception; `TestFieldByteMutationFuzz` does 0-3 byte mutations per iteration against the canonical seed, mirrors the Fase 28 D12 mutator); axon-frontend `tests/fase30_parser_fuzz.rs` (**2/2 pass** with internal 100×10 iteration counts — `random_value_mutation_never_crashes` is byte-identical mirror of the Python class so cross-stack regressions surface on the same seed; `random_byte_mutation_on_axonendpoint_never_crashes` is the Rust counterpart of `TestFieldByteMutationFuzz` using `std::panic::catch_unwind` to surface any panic explicitly); axon-rs `tests/fase30_sse_fuzz.rs` (**4/4 pass**, ~1500 effective wire-format-invariant assertions across iterations — `sse_wire_invariants_hold_under_request_body_fuzz` rolls (flow_name, backend) from adversarial pools and asserts W3C invariants on every 200 OK (retry-first, exactly-once retry, monotonic-id, no-bare-CR, blank-line-terminator); `sse_wire_invariants_hold_under_keepalive_variants` ranges over all 5 declaration shapes (no axonendpoint + 4 keepalive enum values); `negotiation_never_panics_under_accept_header_fuzz` 100 iters across MIME pool + CRLF-injection rejection at hyper layer; `resolve_keepalive_never_panics_under_source_byte_fuzz` 1000 mutations × bounds-check that Duration ∈ {5,15,30,60}s). Reuses `Xorshift` PRNG identical to Fase 28 D12 fuzz mutator. **Combined regression**: Python 268+1s + axon-frontend 10 + axon-rs 49 = 327 Fase 30-specific tests green; full repo regression 1430 axon-rs + 251 axon-frontend + 5385 Python untouched. Workflow uses cache strategy aligned with `fase_28_diagnostics.yml`; integration with `coordinated-release.yml` deferred to 30.i where the cross-stack release lands |
| 30.h Adopter documentation surface | ✅ SHIPPED 2026-05-10 | **993 LOC** new (`docs/ADOPTER_STREAMING.md`) + **64 LOC** extension (`docs/STREAM_EFFECTS.md`) + **39 LOC** cross-link (`docs/ADOPTER_DIAGNOSTICS.md`) | Docs | New `docs/ADOPTER_STREAMING.md` — comprehensive adopter-facing streaming guide with 16-section TOC: § What changed in v1.21.0 (with before/after table), § Quick start (deployable axonendpoint example), § Two streaming patterns (single-shot Pattern A vs two-stage Pattern B, with decision matrix), § The `transport:` + `keepalive:` field surface (with default/use-when tables for every closed-enum value), § SSE wire format spec (5-event surface: retry/token/complete/error/keepalive-comment + 5 framing invariants), § Content-negotiation fallback semantics (with full D4/D5 decision matrix mirroring plan vivo §6.1 verbatim), § Type-checker enforcement rules (the formal 3-disjunct predicate from Fase 30.c), § 4 common compile errors with remediation snippets, § Backwards compatibility matrix (D9 contract surface), § Production deployment cookbook with concrete configs for AWS ALB + Cloudflare + nginx + GCP HTTPS LB + Kubernetes Ingress (5 LB topologies × matching `keepalive:` values per LB idle-timeout), § 7-step troubleshooting checklist (the §13 plan vivo checklist materialized verbatim for adopters), § Client-side EventSource recipes in 4 languages/contexts (Browser `EventSource`, Browser `fetch` + `ReadableStream` for POST, Python `httpx`, Rust `reqwest` + `eventsource-stream`), § LSP / IDE integration recipe, § CI integration recipe, § Cross-stack contract (Python ↔ Rust D7 + transparent disclosure of known Rust frontend parser gaps from 30.e), § Where to file bugs (8-row symptom → tracker mapping), § See also. `docs/STREAM_EFFECTS.md` extended with new "§ HTTP wire format (Fase 30)" section: 4-row quick-mapping table (stream shape × transport declaration × HTTP response), W3C SSE wire spec, where-to-look-in-the-code links, and the load-bearing clarification that backpressure-policy choice does NOT change the wire format (the 4 policies operate at runtime; `fail` reaches the wire only via `axon.error`). `docs/ADOPTER_DIAGNOSTICS.md` extended with new Pattern 5 (Fase 30.c soundness error) immediately after Pattern 4, plus a smart-suggest extension example on `transport:` typo (Levenshtein hint surfaces the closed-enum recovery verbatim) — the cross-link closes the loop so adopters debugging Fase 28 diagnostics find their way to Fase 30 deeper docs naturally. **All anchor cross-references verified** (`STREAM_EFFECTS.md#http-wire-format-fase-30`, `ADOPTER_STREAMING.md#type-checker-enforcement-rules`, etc.) match GitHub-Flavored-Markdown lowercase-hyphen anchor format. Founder principle honored: doc is for axon adopters generally, not for Kivi specifically — every section is adopter-agnostic |
| 30.i Coordinated cross-stack release | ✅ SHIPPED 2026-05-10 | release commit `edee6bb` + 3 annotated tags + 2 crates.io publishes + GitHub Release with 130-line content-first notes | All stacks | **axon-lang v1.21.0 live cross-stack**: PyPI `axon-lang==1.21.0` + crates.io `axon-lang 1.21.0` + crates.io `axon-frontend 0.9.0` + GitHub Release [v1.21.0](https://github.com/Bemarking/axon-lang/releases/tag/v1.21.0). Bump-my-version minor handled the 5 axon-lang files atomically (`pyproject.toml` + `axon/__init__.py` + `axon-rs/Cargo.toml` + `tests/test_cli_mvp_smoke.py` × 2 lockstep asserts + `tests/test_frontend_contract_golden.py`); manual bumps for `axon-frontend/Cargo.toml` (0.8.0 → 0.9.0) + axon-rs dep pin (`axon-frontend = { ..., version = "=0.9.0" }`); `cargo update --workspace` refreshed both lockfiles. 3 annotated tags created + pushed to origin: `v1.21.0` (Python release identifier triggering publish.yml via release-published event), `rust-v1.21.0` (axon-lang Rust crate alias triggering rust_release.yml binary builds + GitHub Release creation), `axon-frontend-v0.9.0` (informational for the frontend crate). Cargo publishes ran in dependency order: `axon-frontend 0.9.0` first (axon-rs depends on it via `=0.9.0` pin), then `axon-lang 1.21.0`. GitHub Release notes rewritten content-first per versioning discipline — 130-line markdown describing the surface added (Parser + Type-checker + Runtime + Adopter docs + CI + D-letter index + Stack matrix + Tests + Founder principle), Fase number only as content tag in the title. Known PyPI publish.yml `release: published` trigger quirk surfaced (memory `project_axon_lang_publish_trigger_quirk.md`) — resolved via documented draft-toggle recovery (`gh release edit --draft && --draft=false`); publish.yml then fired correctly on the re-publish event and PyPI upload completed. Founder principle "axon implementations are for axon as a better language indistintamente de adopters" honored end-to-end — every section of the GitHub Release notes is adopter-agnostic. Fase 30 closes: 9 sub-fases accumulated, 1430+ axon-rs lib regression tests green, 327 Fase 30-specific tests green, cross-stack drift gate operational in CI. **axon-enterprise catch-up follows separately as v1.12.0** (lean bump 2 files + dep pin `axon-lang>=1.20.0` → `>=1.21.0`, same shape as v1.11.0). |
