...
| 24.k Coordinated cross-stack release v1.18.0 | ✅ DONE 2026-05-08 | release | — | bump-my-version 1.17.0 → 1.18.0 cross-stack (pyproject + axon/__init__ + axon-rs/Cargo + golden test pins; axon-frontend stays 0.7.0 — Fase 24 no toca IR shape) + commit `e3f286f` + tag `v1.18.0` + tag `rust-v1.18.0` + push origin master + push tags. **PyPI axon-lang 1.18.0** + **crates.io axon-lang 1.18.0** publicados. GitHub Release v1.18.0 publicado con full release notes (released auto-creó por workflow, edited con notes). PyPI publish trigger quirk recovered via draft toggle (per known memory entry). 4/5 workflows verde immediate (Audit Evidence, axon-frontend Dependency Audit, Rust↔Python IR Parity, PyPI Publish); CI + Rust Release Binaries running at ship time. |
