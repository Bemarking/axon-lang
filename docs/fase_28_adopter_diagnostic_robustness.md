...
status: ✅ CLOSED 2026-05-10 — 28.a-k SHIPPED (D1–D12 ratificadas en bloque); axon-lang v1.20.0 publicado: PyPI + crates.io + axon-frontend 0.8.0 + GitHub Release; tags v1.20.0 / rust-v1.20.0 / axon-frontend-v0.8.0 pushed; coordinated cross-stack release commit `ce8cea9`
...
> **Adopter-facing companion guide:** [ADOPTER_DIAGNOSTICS.md](ADOPTER_DIAGNOSTICS.md)
> — 28.j ship surface; covers every Fase 28 mechanism from the
> outside-in (CLI recipes, JSON schema, LSP recipe, strict opt-in,
> migration path). This document is the internal plan vivo + sub-fase
> tracker; the adopter guide is what links into the public README.
...
| 28.i CI matrix + drift gate + fuzz pack | ✅ SHIPPED 2026-05-10 | ~210 (corpus.json + Python test) + ~120 (Rust integration test) + ~100 (regen script) + ~150 (CI workflow YAML) | YAML + Python + Rust | Single source of truth `tests/fixtures/fase28_drift_gate/corpus.json` (15 entries × 3 expected counts each = 45 contract values). Python pack `tests/test_fase28_drift_gate.py` parametrized over corpus → **51/51 tests** (15×3 count assertions + integrity checks). Rust integration test `axon-frontend/tests/fase28_drift_gate.rs` reads same JSON via serde → **4 tests pass** asserting Rust agrees byte-identically. Regen helper `scripts/fase28_regen_drift_corpus.py` runs Python parser over canonical INPUTS list and writes corpus.json; CI verifies regen is idempotent (committed JSON byte-identical to fresh regen). New CI workflow `.github/workflows/fase_28_diagnostics.yml` with 3 parallel lanes: `python-diagnostics` (full Fase 28 packs + 1000-iter fuzz across Python 3.12+3.13) + `rust-diagnostics` (axon-frontend Fase 28 modules + drift-gate + fuzz across Linux/Windows/macOS) + `drift-gate-parity` (regen idempotence + Python drift-gate test). D7 ratified — byte-identical Python ↔ Rust on every corpus entry; D12 ratified — 1000-iter deterministic fuzz pinned in dedicated lane on both stacks. Verified all CI cargo selectors (`parser::fase28`, `smart_suggest::fase28`, `--test fase28_drift_gate`, `fuzz_recovery_never_crashes`) work locally before push |
| 28.j ADOPTER_DIAGNOSTICS.md guide | ✅ SHIPPED 2026-05-10 | 669 (Markdown) | Docs | New `docs/ADOPTER_DIAGNOSTICS.md` — top-level adopter-facing guide with 13 sections: (1) what changed in v1.20.0 + before/after table; (2) `axon parse` subcommand reference + quick recipes; (3) reading source-context blocks (anatomy + rustc-compatible note); (4) smart-suggest behavior + match-count formats; (5) multi-file aggregation + built-in dir ignores + `.axonignore` cascading + `--ignore` flag; (6) structured JSON schema (per-diagnostic shape + array vs ndjson framing + stable contract); (7) LSP integration recipe with `to_lsp_diagnostic()` helper + severity-int + line/col 0-based conversion; (8) strict opt-in (CLI flag + env var truthy values + what strict does + when to use); (9) common error patterns + fixes (covers v1.19.2/3/4 trilogy + new typo'd-keyword pattern); (10) CI integration cookbook (GitHub Actions + GitLab + jq dashboard + per-PR error-budget guard); (11) programmatic `parse_with_recovery()` API examples (Python + Rust); (12) migration path for existing `parse()` callers; (13) cross-stack contract Python ↔ Rust + drift gate location + bug-routing table. Cross-link added from `README.md` CLI Usage section (5 example invocations) + plan vivo this doc top header. D10 ratified (standalone doc; INTEGRATION_GUIDE.md cross-link target lives in axon-enterprise repo, README + plan vivo cross-links cover the OSS surface) |
| 28.k Coordinated cross-stack release v1.20.0 | ✅ SHIPPED 2026-05-10 | release | — | Coordinated bump axon-lang 1.19.4 → 1.20.0 + axon-frontend 0.7.0 → 0.8.0 + dep pin update in axon-rs/Cargo.toml; release commit `ce8cea9`; 3 tags created + pushed (`v1.20.0` + `rust-v1.20.0` + `axon-frontend-v0.8.0`); cargo publish: axon-frontend 0.8.0 → crates.io ✅ + axon-lang 1.20.0 → crates.io ✅; PyPI publish.yml v1.20.0 conclusion=success after one draft-toggle quirk recovery (per documented quirk pattern); GitHub Release published https://github.com/Bemarking/axon-lang/releases/tag/v1.20.0 with comprehensive notes (TL;DR + 8-row sub-fase delta table + adopter-facing surface + programmatic API + backwards-compat statement + cross-stack contract + test signal + trigger story + install instructions + 12-D ratification list); Fase 28 — Diagnostic Robustness Gate workflow conclusion=success on master + v1.20.0 tag (1m24s + 4m20s) — drift gate locked in CI on first release |
