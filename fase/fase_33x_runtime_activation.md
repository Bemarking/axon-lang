...
| **33.x.m** | release | release | ✅ SHIPPED 2026-05-13 | **axon-lang v1.25.0 cross-stack live.** bump-my-version minor 1.24.0 → 1.25.0 (6 file targets); Cargo.lock regenerated via `cargo check`. crates.io: `axon-lang 1.25.0` published (run 1m). PyPI: `axon-lang 1.25.0` published via publish.yml run 25777793674 (39s) — required draft-toggle quirk recovery to fire `release: published` event. GitHub Release v1.25.0 published with content-first notes covering all 12 sub-fases 33.x.a–33.x.l. axon-frontend stays 0.11.1 (no AST changes). Tags `v1.25.0` + `rust-v1.25.0` pushed via refspec. **axon-enterprise v1.16.0 catch-up live**: PR #18 merged commit `094a6ba` + tag `v1.16.0` via refspec mapping `enterprise/v1.16.0:refs/tags/v1.16.0` + GitHub Release v1.16.0 with vertical-inheritance notes for HIPAA + Legal + Fintech + Government regulated verticals (per-step audit row, cancel-budget guarantee, tokenizer-fallback vertical-aware override hook). Substantive enterprise-only vertical-aware streaming hooks (per-chunk PHI scrubber wired into the runtime chunk path, banking real-streaming audit ledger, legal real-streaming privilege-trace, fintech real-streaming AML signal preservation) deferred to a future enterprise-only release with their own founder sign-off — same lean-catchup shape as v1.9.0 / v1.11.0 / v1.12.0 / v1.13.0 / v1.14.0. |
