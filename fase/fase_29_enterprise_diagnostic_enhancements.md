...
status: ✅ CLOSED 2026-05-12 — Fase 29 cycle fully shipped end-to-end as **axon-enterprise v1.15.0** (PR #17 merged commit `a5c6ab0`, release commit `33bfe4f`; tag `v1.15.0` via refspec mapping `enterprise/v1.15.0:refs/tags/v1.15.0`; [GitHub Release v1.15.0](https://github.com/Bemarking/axon-enterprise/releases/tag/v1.15.0) published with content-first notes). D1–D10 RATIFICADAS bloque; 9 sub-fases 29.a-29.i SHIPPED; **195 diagnostics tests green**; INTEGRATION_GUIDE.md extended +381 LOC with new §10 covering all adopter surfaces. First substantive enterprise-only release since v1.10.0. axon-lang dep pin stays at `>=1.24.0` (no upstream change required). Founder principle honored: "axon-enterprise NO es solo wrapper multitenant; es capa privilegiada con R&D vertical".
...
| 29.g CI matrix: vertical diagnostic gate | ✅ SHIPPED 2026-05-12 (axon-enterprise commit `94becfb` on `feature/29b-vertical-diagnostic-policy`) | ~960 (workflow YAML + curated corpus + 23 corpus drift tests) | YAML + Python | New `.github/workflows/fase_29_vertical_diagnostics.yml` with **6 parallel lanes** (expanded from 3 to cover dashboard + gate + drift + regression anchor): (1) **vertical-policy** Python 3.12 × 3.13 matrix exercising 29.b + closed-catalog assertion; (2) **telemetry-sink** Python 3.12 × 3.13 exercising 29.c + standalone D4 type-level assertion + COMPLIANCE_PARSE_ERROR audit-event catalog membership; (3) **suggest-dict** Python 3.12 × 3.13 exercising 29.d + standalone D8 cross-vertical contamination assertion + raw-file D3 provenance pin; (4) **dashboard-and-gate** Python 3.12 × 3.13 exercising 29.e + 29.f + CLI smoke test + Q5 composite-action YAML lint; (5) **corpus-drift** loads `tests/fixtures/fase29_vertical_corpus.json` + replays every case through live modules; (6) **diagnostics-suite-regression** runs the full Fase 29 test pack as a regression anchor. **Curated corpus** (`fase29_vertical_corpus.json`) is the single source of truth: 4 policy_resolution_cases (every TenantVertical variant) + 4 telemetry_emission_cases (error/warning/hint/generic-no-emit) + 5 suggest_dict_cases (3 verticals + generic-empty + D8 pairwise-disjoint) + 6 compliance_gate_cases (PASS / FAIL_EXCEEDED on errors/warnings/hints / FAIL_INPUT on malformed mode). Each case D-letter anchored. **23 corpus drift tests** including 4 module-load sanity + per-case parametrized tests (`ids=lambda c: c["name"]` for failure traceability) + 1 closed-catalog `test_corpus_covers_every_tenant_vertical` pin. Combined Fase 29 diagnostics: **195 tests green** (29.b 36 + 29.c 22 + 29.d 29 + 29.e 33 + 29.f 52 + 29.g 23). Drift detection works two ways — module shift fails the corpus test with the case name; spec shift fails the module-load sanity tests. |
| 29.h Adopter guide: vertical diagnostic recipes | ✅ SHIPPED 2026-05-12 (axon-enterprise commit `4cb1fe0` on `feature/29b-vertical-diagnostic-policy`) | ~383 LOC Markdown (docs/INTEGRATION_GUIDE.md grew 324 → 705 LOC; +381 inserted, 2 modified for renumbering) | Docs | New §10 "Vertical Diagnostic Policy (v1.15.0+)" extending the existing 11-section adopter guide to 12 sections. **7 subsections** matching the plan vivo recipe: §10.1 tenant vertical resolution + default policies (closed catalog table + bootstrap path); §10.2 opting into/out of strict mode (`DiagnosticPolicy.with_override()` semantics + vertical-vs-override separation); §10.3 telemetry sink config (three-sink fan-out + D4 type-level privacy boundary + `set_audit_sink(StoreBackedAuditSink(...))` recipe); §10.4 consuming `/api/v1/tenant/diagnostics/recent` (aggregated + raw mode curl examples + pagination via `since` + full response shapes + query-param reference); §10.5 CI gate installation (one-line composite-action YAML + full input reference + outputs + closed exit-code catalog + local-run recipe); §10.6 vertical-suggest patterns (per-vertical dictionary metadata table with term counts + sample terms + anchored standards + wiring + provenance inspection); §10.7 D-letter operational index. **D10 enforced**: extension only, no new file; existing §1-§9 numbering preserved (only §10/§11 renumbered to §11/§12 — surgical scope keeps cross-references from sibling docs intact). **D4 reaffirmed**: every response shape + CLI output description carries explicit "NO source text" annotation. **D9 reaffirmed**: section opens with "If your tenant has no registered vertical, every surface in this section behaves identically to the OSS baseline — ignore this section". **Endpoint reference table (§9) extended** with `GET /api/v1/tenant/diagnostics/recent` row cross-referencing §10. |
| 29.i Coordinated release | ✅ SHIPPED 2026-05-12 (axon-enterprise commit `33bfe4f` on `feature/29b-vertical-diagnostic-policy`; PR #17 merged commit `a5c6ab0`; tag `v1.15.0` via refspec `enterprise/v1.15.0:refs/tags/v1.15.0`; [GitHub Release v1.15.0](https://github.com/Bemarking/axon-enterprise/releases/tag/v1.15.0) published) | 2-file version bump + content-first release notes | — | Two-file bump shape per `reference_enterprise_release_workflow` memory: `pyproject.toml` version `1.14.0` → `1.15.0` + `axon_enterprise/__init__.py` `__version__ = "1.15.0"`. axon-lang dep pin **stayed at `>=1.24.0`** (Fase 29 is pure enterprise R&D — no upstream change). PR #17 opened + merged via squash + tag pushed via refspec mapping. GitHub Release published with content-first notes describing the 6 surfaces shipped (vertical-aware policy + telemetry sink + 154-term dictionaries + dashboard endpoint + CI gate + INTEGRATION_GUIDE.md §10) + the closed-catalog D1-D10 D-letter trace + 195-test surface breakdown. First **substantive enterprise-only** release since v1.10.0; the v1.11.0-v1.14.0 cadence was all lean catch-ups consuming upstream axon-lang work (Fase 28→33 cascade). Cadence shape contrast captured verbatim in the release notes so adopters reading the release-history understand the engineering rhythm. |
