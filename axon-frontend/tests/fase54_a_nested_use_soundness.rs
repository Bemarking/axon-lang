//! §Fase 54.a — a `use` nested inside a `step { }` body is a parse error,
...
// ─── §1 — the nested form is now a parse error ──────────────────────
...
fn nested_use_in_step_body_is_a_parse_error() {
    ...
    let err = parse(src).expect_err(
        "a `use` nested inside a step body MUST be rejected — pre-§54.a it was \
...
// ─── §2 — the canonical FLOW-level form still parses ────────────────
...
// ─── §4 — probe / reason / weave / stream still skip structurally ───
...
fn sibling_subconstructs_in_step_body_still_skip_cleanly() {
    ...
    for kw in ["probe Foo", "reason Bar", "weave Baz", "stream Qux"] {
        ...
        assert!(
            parse(&src).is_ok(),
            "`{kw}` nested in a step body must still skip structurally (unchanged by §54.a)"
        );
    }
}
